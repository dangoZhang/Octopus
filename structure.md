...
├── commit 3: record Feed trace and structure evidence
...
| Evolution patch boundary | Normalizes provider patch text, strips apply-patch/markdown wrappers, splits embedded diff headers, strips provider-prefixed patch control lines, inserts missing existing-file and new-file headers, removes diff-boundary blank separators, recounts hunk headers from actual patch body, extracts diff paths, renders display paths, filters authorized patch paths before artifact/apply code can write a patch file, and reports target-boundary violations before live apply. | `evolution_patch.rs` | 673 |
...
| Field adaptation core | Field-pack loading, matching, editable aliases, multilingual alias signals, Need annotation, structured peer-field queue context, trace metadata, peer-field worker slots, verifier results, live field mini task loader, editable field-pack task surfaces with concrete pack and registry target files, repair templates, mini task schema guard, LLM template result normalization, verifier-status normalization, pyfrag prevalidation, compile/execute template checks, verifier artifact propagation, FEED.md status sync, evolved math-mini-7 stationary-point evidence, evolved write-mini-7 claim-map/gap evidence, evolved translate-mini-7 term-preservation evidence, evolved search-mini-7 source-dedupe evidence, evolved code-mini-7 deterministic patch-score evidence, evolved swe-mini-6 invoice-regression evidence, evolved research-mini-6 local-source synthesis evidence, evolved computer-use-mini-6 local-app region evidence, evolved ib-mini-6 stress-matrix evidence, and evolved robotics-mini-6 simulator replan evidence | `field_pack.rs`, `field-packs/**`, `tentacles/field-mini-task/**`, `docs/field-adaptation.md` | 10,073 |
...
- Field evolution proof point: math-mini-5 was generated through `evolve recommend`, failed through apply/check/Feed, repaired in editable harness, then passed Need -> Tentacle -> Feed with trace evidence. Math then evolved to `math-mini-6`, exposed hunk-only existing-file patch output from the LLM planner, added generic patch header normalization, applied the same candidate, repaired the editable template result shape, and passed Need -> Feed with trace #122 and stationary-point evidence under `.octopus/field-mini-task/math/20260701T051444Z-math-mini-6/`. Write then evolved to `write-mini-6`, exposed provider-prefixed new-file control lines that dropped the template, added generic control-line normalization, restored the editable template, and passed Need -> Feed with trace #123 and claim-map evidence under `.octopus/field-mini-task/write/write-mini-6/20260701T052609Z/`. Translate then evolved to `translate-mini-6`, exposed a partial-result metadata failure, self-repaired its runtime template, cleaned stale unreachable code, synced FEED.md to satisfied, propagated verifier artifact metadata, and passed official Need -> Feed with trace #126. Search then evolved to `search-mini-6`, passed `planning -> recommended -> applied -> checking -> need -> feed`, and passed rebuilt-binary Need -> Feed with trace #128 and evidence under `.octopus/field-mini-task/search/20260701T055157Z-search-mini-6/artifacts/search-mini-6/CHECKS.json`. Code then evolved to `code-mini-6`, exposed a bad newline-emission repair, removed the global monkey-patch direction, and passed formal Need -> Feed with trace #131 and endpoint-validation evidence under `.octopus/field-mini-task/code/20260701T060312Z-code-mini-6/`. SWE then evolved to `swe-mini-6`, exposed test-source newline escaping failure, fixed the editable template, and passed formal Need -> Feed with trace #133 and invoice-regression evidence under `.octopus/field-mini-task/swe/20260701T061032Z-swe-mini-6/`. Research then evolved to `research-mini-6`, exposed a partial-gap metadata failure, repaired the editable template, and passed formal Need -> Feed with trace #134 and local-source synthesis evidence under `.octopus/field-mini-task/research/20260701T062105Z-research-mini-6/`.
- Latest self-evolution proof point: math-mini-7, write-mini-7, translate-mini-7, search-mini-7, and code-mini-7 were generated by `evolve drive`. Code first exposed silent planning, then patch-context failures, then a partial Feed from a generated pyfrag indentation bug. The core added observable planning progress/deadline and structured field-pack apply; Octopus then repaired its own `code-mini-7.pyfrag` and reached `planning -> recommended -> applied -> checking -> need -> feed` with Feed trace #142 and verifier #122.
- Latest Go-worker proof point: trace #143 exposed `go_runtime_missing` on `swe-go-default-smoke`; Octopus then generated `tentacles/field-mini-task/workers/swe/swe-go-default-smoke/main.go` through `evolve drive` without creating a new pyfrag. A direct run now selects `./workers/swe/swe-go-default-smoke` and returns honest partial on this no-Go machine. Trace #144 exposed a separate framework issue: the post-apply auto Feed queued generic SWE context without `field_mini_task`, producing unsupported Feed instead of rerunning the repaired worker. The evolution feed cycle now derives a targeted rerun from the source trace metadata, so runtime-created Go workers can be verified without being present in the static field-pack table.
...
- Latest targeted Feed proof point: after retry evidence improved, a live `evolve drive` applied the SWE Go worker patch, ran checks, queued `Rerun swe mini task swe-go-default-smoke after harness evolution`, and produced trace #145 with `field_mini_task=swe-go-default-smoke`. The desktop pet state moved to `harness/partial`, not stale `blocked`; the remaining gap is the local missing Go runtime and explicit artifact-backed partial evidence.
...
- If field-pack evolution proposes `task_layers`, object-shaped `mini_tasks`, patch text with a nested `diff --git`, a syntactically applied but invalid field-pack JSON, or a repair template id missing from `mini_tasks`, inspect `evolution_prompt.rs`, `evolution_candidate.rs`, `evolution_patch.rs`, and `evolution_apply.rs`. The planner should see `field-packs/field-pack.schema.json`, field-pack tail context should show the array closing point, bad schema shapes should be rejected before apply, embedded diff headers and bad hunk counts should be normalized before `git apply`, dry-run validation should reject broken field-pack/template contracts before live apply, template-only repairs should copy the current field-pack into the dry-run worktree, and post-apply schema failures should be reversed with `field_pack_schema_failed`.
- If `octopus evolve` stays in planning, run with explicit bounds such as `OCTOPUS_EVOLVE_PLANNING_TIMEOUT=180 OCTOPUS_EVOLVE_PLANNING_PROGRESS_SECONDS=15`. A timeout must become a fresh `blocked` pet event with `stage=planning` or `stage=apply_retry_planning` and `error_class=provider_timeout`; if it does not, inspect `observation_contract.rs`, `evolution_driver.rs`, `evolution_surface.rs`, `evolution_llm_plan.rs`, and `llm_provider.rs`.
...
- Current field-mini-task template coverage is 65/65 satisfied after code-mini-7. The apply path normalizes LLM patch file headers, strips provider-prefixed new-file control lines, strips patch wrappers before `git apply`, inserts missing existing-file headers, removes diff-boundary blank lines, recounts hunk headers, relocates exact-content hunks when provider line numbers drift, structurally appends provider-authored field-pack mini tasks when stale JSON context blocks `git apply`, rejects field-pack schema-shape and orphan harder-layer mistakes before apply, runs an apply precheck first, reverses post-apply invalid field-pack JSON, retries with current-file context after patch failure, and field-mini-task normalizes common LLM template result shapes into `octopus-json-v1` Feed. Math, search, code, write, and translate are at mini-7; swe, research, computer-use, ib, and robotics are at mini-6; the next concrete curriculum field is `swe`.
