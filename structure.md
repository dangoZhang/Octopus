...
Updated: 2026-07-01, after field-pack dry-run validation plus math/write/translate mini-7 self-evolution.
...
| Editable tentacles | Code-as-harness Feed suppliers: prompts, manifests, tools, declared evolution requirements, field-pack task targets, repair templates, repair surfaces | `tentacles/**` | 22,853 |
...
- Latest self-evolution proof point: math-mini-7, write-mini-7, and translate-mini-7 were generated by `evolve drive`. Translate first blocked at `stage=applying` with `patch_check_failed`, the pet showed the failure, then a repair objective using the current JSON tail reached `planning -> recommended -> applied -> checking -> need -> feed`. The three runs produced Feed traces #138/#139/#140 plus verifiers #118/#119/#120 with artifact evidence under `.octopus/field-mini-task/`.
...
| Area | Files | Lines |
| --- | ---: | ---: |
| `crates/octopus-core/src` | 58 | 61,709 |
| `crates/octopus-core/examples` | 0 | 0 |
| `tentacles` | 105 | 22,853 |
| `field-packs` | 14 | 748 |
| `desktop/pet` | 1 | 841 |
