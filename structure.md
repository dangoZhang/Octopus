...
Updated: 2026-06-30, after modular evolution/pet diagnostics refactor and LLM evolution planner/contract/recommend/prompt/candidate/artifact/patch/target plus tentacle scaffold, brain loop, Need queue, state-report, LLM provider, and manifest tentacle runtime extraction.
...
│   └── src/
│       ├── lib.rs             Kernel contracts, state mutation, manifest data, routes, traces, public kernel API.
...
| --- | --- | --- | ---: |
| Stable kernel | Goal/Need/Feed contracts, state mutation, manifest data, route scores, memory, Feed traces, and public kernel API | `crates/octopus-core/src/lib.rs` | 9,358 |
...
- `lib.rs` still owns broad state mutation and manifest data/profile install APIs. The next cleanup should split durable state mutation or manifest loading/registry before touching product backend behavior.
...
| Area | Files | Lines |
| --- | ---: | ---: |
| `crates/octopus-core/src` | 38 | 58,778 |
...
- Cleanup audit after the evolution-boundary commits moved prompt, candidate, artifact, patch, target, contract, recommendation, LLM planner, scaffold, clean-brain loop, Need queue, state-report, LLM provider, and manifest runtime responsibilities out of `lib.rs`. Remaining discomfort: `main.rs` is still product-backend aggregation, and `lib.rs` still owns broad state mutation plus manifest data/profile install APIs.
