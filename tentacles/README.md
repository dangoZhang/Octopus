...
- `field-mini-task`: generic field-task execution surface. It records peer-field worker tasks, expected Feed, and artifact paths so Octopus can evolve domain behavior from real traces; `repair-templates/` keeps task-specific runtime fragments outside the Rust core.
