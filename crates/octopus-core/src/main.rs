use octopus_core::{
    default_field_pack_catalog, field_pack_report, select_field_pack, user_surface,
};
use octopus_core::{
    ...
    think_tentacle_with_llm_factory, write_tentacle_apply_artifacts,
    ...
    GoalNeedSuggestion, GoalRefinement, Harness, HarnessLearningSummary, HarnessState,
    HeartbeatReport, InstalledTentacle, LoadedTentacleManifest, Need, NeedKind, NeedQueueItem,
    NeedQueueReport, NeedQueueSaveReport, NeedQueueTakeReport, OpenAiCompatibleChatClient,
    OpenAiCompatibleConfig, OpenAiCompatibleTuning, ParallelEvolutionRun, RepairOutcome,
...
use evolution_surface::*;
use feedback_surface::*;
use field_surface::*;
use heartbeat_surface::*;
use llm_layers::*;
use need_runner::{
    empty_parallel_evolution_batch_report, parallel_evolution_next_actions,
    parse_parallel_evolution_args, parse_worker_cap, record_auto_field_verifier_from_feed,
    record_parallel_evolution_action_event, run_queued_need_indices_with_observer_state,
};
use need_surface::*;
#[cfg(test)]
use pet::percent_encode_path;
use pet::{default_pet_image_path, write_pet_image_report};
use pet_surface::*;
use preflight_surface::*;
use product_surface::*;
use profile_registry::{profile_registry_report, ProfileRegistryReport};
use provider_surface::*;
use release_gate::{
...
#[derive(Debug, serde::Serialize)]
struct BrainCouncilReport {
    policy: String,
    prompt: String,
    prefixes: Vec<String>,
    draft_count: usize,
    drafts: Vec<BrainSynthesisDraft>,
    synthesis: BrainSynthesisReport,
    next: Vec<String>,
}

#[derive(Debug, serde::Serialize)]
struct BrainCouncilSaveReport {
    council: BrainCouncilReport,
...
fn run(args: Vec<String>) -> Result<(), String> {
    ...
    match rest.first().map(String::as_str) {
        ...
        Some("repair") => handle_repair_command(&rest, &state, json, language),
        Some("think") => {
        ...
        Some("brain") => handle_brain_command(&rest, &state, json, language),
        Some("explore") => {
        ...
        Some("needs") => handle_needs_command(&rest, &state, json, language),
        Some("llm") => {
            let message = rest
                .get(1..)
                .filter(|values| !values.is_empty())
        ...
        Some("benchmark") => {
            if rest.get(1).map(String::as_str) == Some("check") {
                ...
                let report = check_benchmark_record(&path, git_short_head())?;
                if json {
                    println!(
                        "{}",
                        serde_json::to_string_pretty(&report).map_err(|error| error.to_string())?
            ...
            let path = rest
                ...
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from(DEFAULT_BENCHMARK_RECORD_PATH));
            let report = write_benchmark_record(
                &path,
        ...
        Some("goal") => handle_goal_command(&rest, &state, json, language),
        Some("context") => {
            let loaded = HarnessState::load(&state).map_err(|error| error.to_string())?;
            ...
            if rest.get(1).map(String::as_str) == Some("record") {
                ...
                if rest.get(2).map(String::as_str) == Some("check") {
                    let path = rest
                        ...
                        .map(PathBuf::from)
                        .unwrap_or_else(|| PathBuf::from(".octopus/real-machine-record.md"));
                    let report = check_preflight_record(&path)?;
                    if json {
                        println!(
                            "{}",
                            serde_json::to_string_pretty(&report)
                        ...
                        print_preflight_record_check(&report, language);
                    }
                    return Ok(());
                }
                let path = rest
        ...
        Some("beat") => handle_beat_command(&rest, &state, json, language),
        Some("oauth") => {
            let provider = rest
                .get(1)
        ...
        Some("evolve") => handle_evolve_command(&rest, &state, json, language),
        Some("scaffold") => {
            let tentacle_id = rest
                .get(1)
...
# export OCTOPUS_BRAIN_INTENT_LLM_PREFIX=OCTOPUS_LLM
# export OCTOPUS_BRAIN_BRIEF_LLM_PREFIX=OCTOPUS_LLM
# export OCTOPUS_BRAIN_ALIGN_LLM_PREFIX=OCTOPUS_LLM
# export OCTOPUS_BRAIN_SCOUT_LLM_PREFIX=OCTOPUS_LLM
...
fn write_brain_deliberation_session(
    state: &HarnessState,
    state_path: &Path,
    prompt: &str,
    live: bool,
    ...
    let apply_command = format!(
        "octopus --state {state_arg} brain --deliberate --apply {reply_arg} --save {prompt_arg}"
    );

    fs::write(
        &prompt_path,
        brain_session_prompt_markdown(&prompt_report, &messages, "deliberate", &apply_command),
    )
    .map_err(|error| error.to_string())?;
    fs::write(
        &messages_path,
    ...
    fs::write(
        &command_path,
        format!(
            "#!/usr/bin/env sh\nset -eu\n# Paste the accepted deliberation JSON into REPLY.json first.\n{apply_command}\n"
        ),
    )
    .map_err(|error| error.to_string())?;
    ...
    if draft_path_string.is_some() {
        ...
        next.push("paste PROMPT.md or messages.json into a chat model".to_string());
        next.push("replace REPLY.json with deliberation JSON".to_string());
    }
    next.push(apply_command.clone());
...
fn write_brain_clarification_session(
    state: &HarnessState,
    state_path: &Path,
    prompt: &str,
    live: bool,
    ...
    let prompt_report = state.clean_brain_prompt(prompt.to_string(), 6);
    let messages = brain_clarification_session_messages(&prompt_report);
    let reply_template = serde_json::json!({
        "summary": "short clarification",
        "observations": ["cognitive observation"],
    ...
    let reply_arg = shell_arg(reply_path.to_string_lossy().as_ref());
    let apply_command = format!(
        "octopus --state {state_arg} brain --clarify --apply {reply_arg} --save {prompt_arg}"
    );

    fs::write(
        &prompt_path,
        brain_session_prompt_markdown(&prompt_report, &messages, "clarify", &apply_command),
    )
    .map_err(|error| error.to_string())?;
    fs::write(
        &messages_path,
    ...
    fs::write(
        &command_path,
        format!(
            "#!/usr/bin/env sh\nset -eu\n# Paste the accepted clarification JSON into REPLY.json first.\n{apply_command}\n"
        ),
    )
    .map_err(|error| error.to_string())?;
    make_executable(&command_path)?;
    ...
    if draft_path_string.is_some() {
        ...
        next.push("paste PROMPT.md or messages.json into a chat model".to_string());
        next.push("replace REPLY.json with clarification JSON".to_string());
    }
    next.push(apply_command.clone());
...
fn write_brain_agenda_session(
    ...
    let prompt_report = state.clean_brain_prompt(prompt.to_string(), 6);
    let messages = brain_agenda_session_messages(&prompt_report);
    let reply_template = serde_json::json!({
        "summary": "short agenda",
        "observations": ["cognitive observation"],
        "questions": ["open cognitive question"],
        "options": ["possible cognitive direction"],
        "risks": ["reasoning risk"],
    ...
    let reply_arg = shell_arg(reply_path.to_string_lossy().as_ref());
    let apply_command = format!(
        "octopus --state {state_arg} brain --agenda --apply {reply_arg} --save {prompt_arg}"
    );

    fs::write(
        &prompt_path,
        brain_session_prompt_markdown(&prompt_report, &messages, "agenda", &apply_command),
    )
    .map_err(|error| error.to_string())?;
    fs::write(
        &messages_path,
    ...
    fs::write(
        &command_path,
        format!(
            "#!/usr/bin/env sh\nset -eu\n# Paste the accepted agenda JSON into REPLY.json first.\n{apply_command}\n"
        ),
    )
    .map_err(|error| error.to_string())?;
    make_executable(&command_path)?;
    ...
    if draft_path_string.is_some() {
        ...
        next.push("paste PROMPT.md or messages.json into a chat model".to_string());
        next.push("replace REPLY.json with agenda JSON".to_string());
    }
    next.push(apply_command.clone());

    Ok(BrainSessionReport {
        policy: prompt_report.policy,
        mode: "agenda".to_string(),
        prompt: prompt.to_string(),
        session_dir: session_dir.to_string_lossy().to_string(),
        prompt_path: prompt_path.to_string_lossy().to_string(),
        messages_path: messages_path.to_string_lossy().to_string(),
...
fn write_brain_scout_session(
    ...
    let prompt_report = state.clean_brain_prompt(prompt.to_string(), 6);
    let messages = brain_scout_session_messages(&prompt_report);
    let reply_template = serde_json::json!({
        "summary": "short scout map",
        "observations": ["cognitive signal"],
        "questions": ["important unknown"],
        "options": ["possible cognitive direction"],
        "risks": ["drift or blind spot"],
        "needs": [
            {"kind": "observe", "query": "short cognitive request"}
        ]
    });
    ...
    let reply_arg = shell_arg(reply_path.to_string_lossy().as_ref());
    let apply_command = format!(
        "octopus --state {state_arg} brain --scout --apply {reply_arg} --save {prompt_arg}"
    );

    fs::write(
        &prompt_path,
        brain_session_prompt_markdown(&prompt_report, &messages, "scout", &apply_command),
    )
    .map_err(|error| error.to_string())?;
    fs::write(
        &messages_path,
    ...
    fs::write(
        &command_path,
        format!(
            "#!/usr/bin/env sh\nset -eu\n# Paste the accepted scout JSON into REPLY.json first.\n{apply_command}\n"
        ),
    )
    .map_err(|error| error.to_string())?;
    make_executable(&command_path)?;
    ...
    if draft_path_string.is_some() {
        ...
        next.push("paste PROMPT.md or messages.json into a chat model".to_string());
        next.push("replace REPLY.json with scout JSON".to_string());
    }
    next.push(apply_command.clone());

    Ok(BrainSessionReport {
        policy: prompt_report.policy,
        mode: "scout".to_string(),
        prompt: prompt.to_string(),
        session_dir: session_dir.to_string_lossy().to_string(),
        prompt_path: prompt_path.to_string_lossy().to_string(),
        messages_path: messages_path.to_string_lossy().to_string(),
...
fn write_brain_reflection_session(
    state: &HarnessState,
    state_path: &Path,
    prompt: &str,
    live: bool,
    ...
    let prompt_report = state.clean_brain_prompt(prompt.to_string(), 6);
    let messages = brain_reflection_session_messages(&prompt_report);
    let reply_template = serde_json::json!({
        "summary": "short reflection",
        "goal_state": "uncertain",
        "evidence": ["evidence from Goal/Mem/Need/Feed"],
        "gaps": ["remaining cognitive gap"],
        "questions": ["open cognitive question"],
        "needs": [
            {"kind": "verify", "query": "short cognitive request"}
        ]
    ...
    let reply_arg = shell_arg(reply_path.to_string_lossy().as_ref());
    let apply_command = format!(
        "octopus --state {state_arg} brain --reflect --apply {reply_arg} --save {prompt_arg}"
    );

    fs::write(
        &prompt_path,
        brain_session_prompt_markdown(&prompt_report, &messages, "reflect", &apply_command),
    )
    .map_err(|error| error.to_string())?;
    fs::write(
        &messages_path,
    ...
    fs::write(
        &command_path,
        format!(
            "#!/usr/bin/env sh\nset -eu\n# Paste the accepted reflection JSON into REPLY.json first.\n{apply_command}\n"
        ),
    )
    .map_err(|error| error.to_string())?;
    make_executable(&command_path)?;
    ...
    if draft_path_string.is_some() {
        ...
        next.push("paste PROMPT.md or messages.json into a chat model".to_string());
        next.push("replace REPLY.json with reflection JSON".to_string());
    }
    next.push(apply_command.clone());
...
fn write_brain_alignment_session(
    state: &HarnessState,
    state_path: &Path,
    prompt: &str,
    live: bool,
    ...
    let prompt_report = state.clean_brain_prompt(prompt.to_string(), 6);
    let messages = brain_alignment_session_messages(&prompt_report);
    let reply_template = serde_json::json!({
        "summary": "short alignment check",
        "goal_state": "uncertain",
        "evidence": ["evidence from Goal/Mem/Need/Feed"],
        "gaps": ["alignment gap"],
        "questions": ["goal-facing question"],
        "needs": [
            {"kind": "verify", "query": "whether current direction follows the Goal and constraints"}
        ]
    });
    let session_dir = next_brain_session_dir(state_path)?;
    fs::create_dir_all(&session_dir).map_err(|error| error.to_string())?;
    ...
    let reply_arg = shell_arg(reply_path.to_string_lossy().as_ref());
    let apply_command = format!(
        "octopus --state {state_arg} brain --align --apply {reply_arg} --save {prompt_arg}"
    );

    fs::write(
        &prompt_path,
        brain_session_prompt_markdown(&prompt_report, &messages, "align", &apply_command),
    )
    .map_err(|error| error.to_string())?;
    fs::write(
        &messages_path,
    ...
    fs::write(
        &command_path,
        format!(
            "#!/usr/bin/env sh\nset -eu\n# Paste the accepted alignment JSON into REPLY.json first.\n{apply_command}\n"
        ),
    )
    .map_err(|error| error.to_string())?;
    make_executable(&command_path)?;
    ...
    if draft_path_string.is_some() {
        ...
        next.push("paste PROMPT.md or messages.json into a chat model".to_string());
        next.push("replace REPLY.json with alignment JSON".to_string());
    }
    next.push(apply_command.clone());

    Ok(BrainSessionReport {
        policy: prompt_report.policy,
        mode: "align".to_string(),
        prompt: prompt.to_string(),
        session_dir: session_dir.to_string_lossy().to_string(),
        prompt_path: prompt_path.to_string_lossy().to_string(),
        messages_path: messages_path.to_string_lossy().to_string(),
...
fn write_brain_memory_session(
    ...
    let prompt_report = state.clean_brain_prompt(prompt.to_string(), 6);
    let messages = brain_memory_session_messages(&prompt_report);
    let reply_template = serde_json::json!({
        "summary": "short memory focus",
        "needs": [
            {"kind": "remember", "query": "durable cognitive context"},
            {"kind": "recall", "query": "relevant memory"},
            {"kind": "forget", "query": "stale cognitive context"}
        ]
    });
    let session_dir = next_brain_session_dir(state_path)?;
    fs::create_dir_all(&session_dir).map_err(|error| error.to_string())?;
    ...
    let reply_arg = shell_arg(reply_path.to_string_lossy().as_ref());
    let apply_command = format!(
        "octopus --state {state_arg} brain --memory --apply {reply_arg} --save {prompt_arg}"
    );

    fs::write(
        &prompt_path,
        brain_session_prompt_markdown(&prompt_report, &messages, "memory", &apply_command),
    )
    .map_err(|error| error.to_string())?;
    fs::write(
        &messages_path,
    ...
    fs::write(
        &command_path,
        format!(
            "#!/usr/bin/env sh\nset -eu\n# Paste the accepted memory JSON into REPLY.json first.\n{apply_command}\n"
        ),
    )
    .map_err(|error| error.to_string())?;
    make_executable(&command_path)?;
    ...
    if draft_path_string.is_some() {
        ...
        next.push("paste PROMPT.md or messages.json into a chat model".to_string());
        next.push("replace REPLY.json with memory JSON".to_string());
    }
    next.push(apply_command.clone());
...
fn write_brain_intent_session(
    ...
    let prompt_report = state.clean_brain_prompt(prompt.to_string(), 6);
    let messages = brain_intent_session_messages(&prompt_report);
    let reply_template = serde_json::json!({
        "summary": "short intent map",
        "needs": [
            {"kind": "observe", "query": "current cognitive uncertainty"},
            {"kind": "verify", "query": "evidence needed for the goal"},
            {"kind": "compare", "query": "best next cognitive direction"}
        ]
    });
    let session_dir = next_brain_session_dir(state_path)?;
    fs::create_dir_all(&session_dir).map_err(|error| error.to_string())?;
    ...
    let reply_arg = shell_arg(reply_path.to_string_lossy().as_ref());
    let apply_command = format!(
        "octopus --state {state_arg} brain --intent --apply {reply_arg} --save {prompt_arg}"
    );

    fs::write(
        &prompt_path,
        brain_session_prompt_markdown(&prompt_report, &messages, "intent", &apply_command),
    )
    .map_err(|error| error.to_string())?;
    fs::write(
        &messages_path,
    ...
    fs::write(
        &command_path,
        format!(
            "#!/usr/bin/env sh\nset -eu\n# Paste the accepted intent JSON into REPLY.json first.\n{apply_command}\n"
        ),
    )
    .map_err(|error| error.to_string())?;
    make_executable(&command_path)?;
    ...
    if draft_path_string.is_some() {
        ...
        next.push("paste PROMPT.md or messages.json into a chat model".to_string());
        next.push("replace REPLY.json with intent JSON".to_string());
    }
    next.push(apply_command.clone());
...
fn write_brain_brief_session(
    ...
    let prompt_report = state.clean_brain_prompt(prompt.to_string(), 6);
    let messages = brain_brief_session_messages(&prompt_report);
    let reply_template = serde_json::json!({
        "summary": "short cognitive brief",
        "needs": [
            {"kind": "observe", "query": "compact current cognitive state"},
            {"kind": "verify", "query": "which brief claims are supported"},
            {"kind": "compare", "query": "which context should guide the next Need"}
        ]
    });
    let session_dir = next_brain_session_dir(state_path)?;
    fs::create_dir_all(&session_dir).map_err(|error| error.to_string())?;
    ...
    let reply_arg = shell_arg(reply_path.to_string_lossy().as_ref());
    let apply_command = format!(
        "octopus --state {state_arg} brain --brief --apply {reply_arg} --save {prompt_arg}"
    );

    fs::write(
        &prompt_path,
        brain_session_prompt_markdown(&prompt_report, &messages, "brief", &apply_command),
    )
    .map_err(|error| error.to_string())?;
    fs::write(
        &messages_path,
    ...
    fs::write(
        &command_path,
        format!(
            "#!/usr/bin/env sh\nset -eu\n# Paste the accepted brief JSON into REPLY.json first.\n{apply_command}\n"
        ),
    )
    .map_err(|error| error.to_string())?;
    make_executable(&command_path)?;
    ...
    if draft_path_string.is_some() {
        ...
        next.push("paste PROMPT.md or messages.json into a chat model".to_string());
        next.push("replace REPLY.json with brief JSON".to_string());
    }
    next.push(apply_command.clone());
...
fn run_brain_council(
    ...
    for prefix in &prefixes {
        ...
        let draft = parse_brain_reply::<BrainDeliberationDraft>(
            &response.content,
            &format!("clean-brain council {prefix}"),
        )?;
        drafts.push(BrainSynthesisDraft {
            source: Some(prefix.clone()),
    ...
    let input = BrainSynthesisInput {
        summary: Some(format!(
            "clean-brain council from {} model(s)",
            drafts.len()
        )),
    ...
    let synthesis = if synthesize_live {
        let mut client = clean_brain_synthesize_llm_client()?;
        state.clean_brain_synthesize_with_client(prompt.to_string(), 6, input, &mut client)?
    } else {
    ...
    next.sort();
    next.dedup();
    Ok(BrainCouncilReport {
        policy: synthesis.policy.clone(),
        prompt: prompt.to_string(),
        prefixes,
...
fn save_brain_council(
    state: &mut HarnessState,
    report: BrainCouncilReport,
) -> BrainCouncilSaveReport {
    let saved = state.queue_synthesis_report(&report.synthesis);
    BrainCouncilSaveReport {
        council: report,
        queued: saved.queued,
        queue: saved.queue,
...
fn brain_deliberation_session_messages(report: &BrainPromptReport) -> Vec<ChatMessage> {
    let context = serde_json::json!({
        "policy": report.policy,
        "slots": ["Goal", "Mem", "Need", "Feed"],
        "goal": report.goal,
...
fn brain_agenda_session_messages(report: &BrainPromptReport) -> Vec<ChatMessage> {
    let context = serde_json::json!({
        "policy": report.policy,
        "slots": ["Goal", "Mem", "Need", "Feed"],
        "goal": report.goal,
    ...
    vec![
        ChatMessage::new(
            ChatRole::System,
            "You are the Octopus clean-brain agenda layer. You see only Goal, Mem, Need, and Feed. Build a compact cognitive agenda that prioritizes what should be understood, verified, remembered, recalled, compared, or clarified next. Do not choose tools, APIs, files, commands, routes, tentacles, or implementation. Return only JSON: {\"summary\":\"short agenda\",\"observations\":[\"cognitive observation\"],\"questions\":[\"open cognitive question\"],\"options\":[\"possible cognitive direction\"],\"risks\":[\"reasoning risk\"],\"needs\":[{\"kind\":\"observe|verify|reproduce|compare|remember|forget|recall|execute\",\"query\":\"short cognitive request\"}]}",
        ),
        ChatMessage::new(
            ChatRole::User,
            format!(
...
fn brain_scout_session_messages(report: &BrainPromptReport) -> Vec<ChatMessage> {
    let context = serde_json::json!({
        "policy": report.policy,
        "slots": ["Goal", "Mem", "Need", "Feed"],
        "goal": report.goal,
...
fn brain_clarification_session_messages(report: &BrainPromptReport) -> Vec<ChatMessage> {
    let context = serde_json::json!({
        "policy": report.policy,
        "slots": ["Goal", "Mem", "Need", "Feed"],
        "goal": report.goal,
    ...
    vec![
        ChatMessage::new(
            ChatRole::System,
            "You are the Octopus clean-brain clarification layer. You see only Goal, Mem, Need, and Feed. Ask the few human-facing questions that would make the Goal and next cognitive Needs clearer. Express cognitive Needs only. Do not choose tools, APIs, files, commands, routes, tentacles, or implementation. Return only JSON: {\"summary\":\"short clarification\",\"observations\":[\"cognitive observation\"],\"questions\":[\"question for the human\"],\"options\":[\"possible cognitive direction\"],\"risks\":[\"ambiguity risk\"],\"needs\":[{\"kind\":\"observe|verify|reproduce|compare|remember|forget|recall|execute\",\"query\":\"short cognitive request\"}]}",
        ),
        ChatMessage::new(
            ChatRole::User,
            format!(
...
fn brain_reflection_session_messages(report: &BrainPromptReport) -> Vec<ChatMessage> {
    let context = serde_json::json!({
        "policy": report.policy,
        "slots": ["Goal", "Mem", "Need", "Feed"],
        "goal": report.goal,
...
fn brain_alignment_session_messages(report: &BrainPromptReport) -> Vec<ChatMessage> {
    let context = serde_json::json!({
        "policy": report.policy,
        "slots": ["Goal", "Mem", "Need", "Feed"],
        "goal": report.goal,
...
fn brain_memory_session_messages(report: &BrainPromptReport) -> Vec<ChatMessage> {
    let context = serde_json::json!({
        "policy": report.policy,
        "slots": ["Goal", "Mem", "Need", "Feed"],
        "goal": report.goal,
...
fn brain_intent_session_messages(report: &BrainPromptReport) -> Vec<ChatMessage> {
    let context = serde_json::json!({
        "policy": report.policy,
        "slots": ["Goal", "Mem", "Need", "Feed"],
        "goal": report.goal,
...
fn brain_brief_session_messages(report: &BrainPromptReport) -> Vec<ChatMessage> {
    let context = serde_json::json!({
        "policy": report.policy,
        "slots": ["Goal", "Mem", "Need", "Feed"],
        "goal": report.goal,
...
fn resolve_feed_trace_selector(value: &str, state: &HarnessState) -> Result<u64, String> {
    if value == "latest" {
        state
            .feed_traces
            .last()
            .map(|trace| trace.index)
            .ok_or_else(|| "no feed traces available".to_string())
    } else {
        parse_trace_index(value)
    }
}
...
mod tests {
    use super::{
        ...
        ensure_field_mini_task_tentacle_from_root, field_pool_parallel_run_record_ready,
        field_pool_status_line, field_trajectory_policy_line, field_trajectory_worker_slot_line,
        first_run_report, http_content_length,
    ...
    use crate::need_runner::{
        contains_field_mini_marker, empty_parallel_evolution_batch_report,
        field_hint_from_queue_item, field_hint_ids, field_mini_task_context,
        next_need_run_worker_count, parallel_evolution_next_actions, parse_need_run_request,
        parse_parallel_evolution_args, parse_worker_cap, record_auto_field_verifier_from_feed,
        record_parallel_evolution_action_event, run_queued_need_indices_with_observer_state,
        run_queued_need_with_observer_state, run_queued_needs_with_observer_state,
        NeedRunBatchReport, NeedRunReport, NeedRunRequest, NeedRunSelector,
    };
    use octopus_core::{
        default_field_pack_catalog, default_field_pack_ids, default_tentacle_profiles,
        load_tentacle_manifests, load_tentacle_profiles_from_path, parallel_field_pool_policy,
        parallel_worker_policy, CheckHistoryInput, Evidence, Feed, FieldTrajectoryReport,
        FieldTrajectorySummary, Goal, GoalNeedSuggestion, GoalStatus, HarnessState,
        InstalledTentacle, Need, NeedKind, NeedQueueItem, NeedQueueStatus, NeedQueueTakeReport,
    ...
    fn need_run_source_saves_need_before_feed_and_feed_after() {
        let source =
            fs::read_to_string(Path::new(env!("CARGO_MANIFEST_DIR")).join("src/need_runner.rs"))
                .unwrap();
    ...
    fn cli_brain_intent_queues_cognitive_needs_without_feed() {
        let _env = env_guard();
        let path = std::env::temp_dir().join(format!(
            "octopus-brain-intent-state-{}.json",
            std::process::id()
        ));
        let state = path.to_string_lossy().to_string();
        let _ = fs::remove_file(&path);
    ...
    fn cli_brain_brief_queues_cognitive_needs_without_feed() {
        let _env = env_guard();
        let path = std::env::temp_dir().join(format!(
            "octopus-brain-brief-state-{}.json",
            std::process::id()
        ));
        let state = path.to_string_lossy().to_string();
        let _ = fs::remove_file(&path);
...
"#,
    ...
    #[test]
    fn cli_brain_deliberate_session_writes_external_chat_artifacts_without_feed() {
        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-deliberate-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        let reply = fs::read_to_string(session.join("REPLY.json")).unwrap();
        let command = fs::read_to_string(session.join("COMMANDS.sh")).unwrap();
        assert!(prompt.contains("mode: deliberate"));
        assert!(messages.contains("deliberation layer"));
        assert!(messages.contains("Goal + Mem + Need + Feed"));
        assert!(reply.contains("\"observations\""));
        assert!(reply.contains("\"questions\""));
        assert!(command.contains("brain --deliberate --apply"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_deliberate_apply_saves_clean_needs_without_feed() {
        let _env = env_guard();
        let dir = std::env::temp_dir().join(format!(
            "octopus-brain-deliberate-apply-{}",
            std::process::id()
        ));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        ...
        run(vec![
            ...
            "--deliberate".to_string(),
            "--apply-json".to_string(),
            "{\"summary\":\"external deliberation\",\"observations\":[\"goal is broad\"],\"questions\":[\"what evidence matters?\"],\"options\":[\"verify progress\"],\"risks\":[\"tool leakage\"],\"needs\":[{\"kind\":\"verify\",\"query\":\"whether goal evidence is enough\"},{\"kind\":\"execute\",\"query\":\"cargo test -p octopus-core\"}]}".to_string(),
            "--save".to_string(),
            "think".to_string(),
            "before".to_string(),
            "Needs".to_string(),
    ...
    #[test]
    fn cli_brain_clarify_session_writes_external_chat_artifacts_without_feed() {
        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-clarify-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        let reply = fs::read_to_string(session.join("REPLY.json")).unwrap();
        let command = fs::read_to_string(session.join("COMMANDS.sh")).unwrap();
        assert!(prompt.contains("mode: clarify"));
        assert!(messages.contains("clarification layer"));
        assert!(messages.contains("Goal + Mem + Need + Feed"));
        assert!(reply.contains("\"questions\""));
        assert!(command.contains("brain --clarify --apply"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_clarify_apply_saves_clean_needs_without_feed() {
        let _env = env_guard();
        let dir = std::env::temp_dir().join(format!(
            "octopus-brain-clarify-apply-{}",
            std::process::id()
        ));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        ...
        run(vec![
            ...
            "--clarify".to_string(),
            "--apply-json".to_string(),
            "{\"summary\":\"clarify reply\",\"observations\":[\"goal is broad\"],\"questions\":[\"what outcome proves success?\"],\"options\":[\"tighten goal\"],\"risks\":[\"wrong target\"],\"needs\":[{\"kind\":\"verify\",\"query\":\"success criteria for launch goal\"},{\"kind\":\"execute\",\"query\":\"cargo test -p octopus-core\"}]}".to_string(),
            "--save".to_string(),
            "clarify".to_string(),
            "goal".to_string(),
        ])
        ...
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("clarify reply"));
        assert!(content.contains("success criteria for launch goal"));
        assert!(!content.contains("cargo test"));
        let _ = fs::remove_dir_all(dir);
    }
    ...
    #[cfg(unix)]
    #[test]
    fn cli_brain_clarify_live_uses_slot_without_feed() {
        use std::os::unix::fs::PermissionsExt;

        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-clarify-live-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let curl = dir.join("clarify-curl.sh");
        fs::write(
            &curl,
            r#"#!/bin/sh
printf '%s' '{"choices":[{"message":{"content":"{\"summary\":\"clarify slot reply\",\"observations\":[\"slot observation\"],\"questions\":[\"slot question\"],\"options\":[\"slot option\"],\"risks\":[\"slot risk\"],\"needs\":[{\"kind\":\"verify\",\"query\":\"clarify-slot cognitive need\"}]}"}}]}'
"#,
        )
        .unwrap();
        let mut permissions = fs::metadata(&curl).unwrap().permissions();
        ...
        let old_clarify_prefix = std::env::var("OCTOPUS_BRAIN_CLARIFY_LLM_PREFIX").ok();
        let old_model = std::env::var("OCTOPUS_SLOT_CLARIFY_MODEL").ok();
        let old_base_url = std::env::var("OCTOPUS_SLOT_CLARIFY_BASE_URL").ok();
        let old_api_key = std::env::var("OCTOPUS_SLOT_CLARIFY_API_KEY").ok();
        let old_curl = std::env::var("OCTOPUS_SLOT_CLARIFY_CURL").ok();
        std::env::set_var("OCTOPUS_BRAIN_LLM", "1");
        std::env::set_var("OCTOPUS_BRAIN_LLM_PREFIX", "OCTOPUS_UNUSED_BRAIN");
        std::env::set_var("OCTOPUS_BRAIN_CLARIFY_LLM_PREFIX", "OCTOPUS_SLOT_CLARIFY");
        std::env::set_var("OCTOPUS_SLOT_CLARIFY_MODEL", "clarify-model");
        std::env::set_var(
            "OCTOPUS_SLOT_CLARIFY_BASE_URL",
            "https://clarify.example/v1",
        );
        std::env::remove_var("OCTOPUS_SLOT_CLARIFY_API_KEY");
        std::env::set_var(
            "OCTOPUS_SLOT_CLARIFY_CURL",
            curl.to_string_lossy().to_string(),
        );
        ...
        restore_env("OCTOPUS_BRAIN_CLARIFY_LLM_PREFIX", old_clarify_prefix);
        restore_env("OCTOPUS_SLOT_CLARIFY_MODEL", old_model);
        restore_env("OCTOPUS_SLOT_CLARIFY_BASE_URL", old_base_url);
        restore_env("OCTOPUS_SLOT_CLARIFY_API_KEY", old_api_key);
        restore_env("OCTOPUS_SLOT_CLARIFY_CURL", old_curl);
        ...
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("clarify slot reply"));
        assert!(content.contains("clarify-slot cognitive need"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_agenda_session_writes_external_chat_artifacts_without_feed() {
        let _env = env_guard();
        let dir = std::env::temp_dir().join(format!("octopus-brain-agenda-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        let reply = fs::read_to_string(session.join("REPLY.json")).unwrap();
        let command = fs::read_to_string(session.join("COMMANDS.sh")).unwrap();
        assert!(prompt.contains("mode: agenda"));
        assert!(messages.contains("agenda layer"));
        assert!(messages.contains("Goal + Mem + Need + Feed"));
        assert!(reply.contains("\"questions\""));
        assert!(command.contains("brain --agenda --apply"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_agenda_apply_saves_clean_needs_without_feed() {
        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-agenda-apply-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        run(vec![
            ...
            "--agenda".to_string(),
            "--apply-json".to_string(),
            "{\"summary\":\"agenda reply\",\"observations\":[\"goal has competing priorities\"],\"questions\":[\"what uncertainty matters first?\"],\"options\":[\"verify the riskiest claim\"],\"risks\":[\"scattered agenda\"],\"needs\":[{\"kind\":\"verify\",\"query\":\"highest uncertainty for launch goal\"},{\"kind\":\"execute\",\"query\":\"cargo test -p octopus-core\"}]}".to_string(),
            "--save".to_string(),
            "agenda".to_string(),
            "goal".to_string(),
        ])
        ...
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("agenda reply"));
        assert!(content.contains("highest uncertainty for launch goal"));
        assert!(!content.contains("cargo test"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_scout_session_writes_external_chat_artifacts_without_feed() {
        let _env = env_guard();
        let dir = std::env::temp_dir().join(format!("octopus-brain-scout-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        let reply = fs::read_to_string(session.join("REPLY.json")).unwrap();
        let command = fs::read_to_string(session.join("COMMANDS.sh")).unwrap();
        assert!(prompt.contains("mode: scout"));
        assert!(messages.contains("scout layer"));
        assert!(messages.contains("Goal + Mem + Need + Feed"));
        assert!(reply.contains("\"options\""));
        assert!(reply.contains("important unknown"));
        assert!(command.contains("brain --scout --apply"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_scout_apply_saves_clean_needs_without_feed() {
        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-scout-apply-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        run(vec![
            ...
            "--scout".to_string(),
            "--apply-json".to_string(),
            "{\"summary\":\"scout reply\",\"observations\":[\"goal has unknowns\"],\"questions\":[\"which assumption matters?\"],\"options\":[\"compare directions\"],\"risks\":[\"drift\"],\"needs\":[{\"kind\":\"compare\",\"query\":\"candidate directions for launch goal\"},{\"kind\":\"execute\",\"query\":\"cargo test -p octopus-core\"}]}".to_string(),
            "--save".to_string(),
            "scout".to_string(),
            "goal".to_string(),
        ])
        ...
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("scout reply"));
        assert!(content.contains("candidate directions for launch goal"));
        assert!(!content.contains("cargo test"));
        let _ = fs::remove_dir_all(dir);
    }
    ...
    #[cfg(unix)]
    #[test]
    fn cli_brain_scout_live_uses_slot_without_feed() {
        use std::os::unix::fs::PermissionsExt;

        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-scout-live-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let curl = dir.join("scout-curl.sh");
        fs::write(
            &curl,
            r#"#!/bin/sh
printf '%s' '{"choices":[{"message":{"content":"{\"summary\":\"scout slot reply\",\"observations\":[\"slot observation\"],\"questions\":[\"slot question\"],\"options\":[\"slot option\"],\"risks\":[\"slot risk\"],\"needs\":[{\"kind\":\"compare\",\"query\":\"scout-slot cognitive need\"}]}"}}]}'
"#,
        )
        .unwrap();
        let mut permissions = fs::metadata(&curl).unwrap().permissions();
        ...
        let old_brain = std::env::var("OCTOPUS_BRAIN_LLM").ok();
        let old_brain_prefix = std::env::var("OCTOPUS_BRAIN_LLM_PREFIX").ok();
        let old_scout_prefix = std::env::var("OCTOPUS_BRAIN_SCOUT_LLM_PREFIX").ok();
        let old_model = std::env::var("OCTOPUS_SLOT_SCOUT_MODEL").ok();
        let old_base_url = std::env::var("OCTOPUS_SLOT_SCOUT_BASE_URL").ok();
        let old_api_key = std::env::var("OCTOPUS_SLOT_SCOUT_API_KEY").ok();
        let old_curl = std::env::var("OCTOPUS_SLOT_SCOUT_CURL").ok();
        std::env::set_var("OCTOPUS_BRAIN_LLM", "1");
        std::env::set_var("OCTOPUS_BRAIN_LLM_PREFIX", "OCTOPUS_UNUSED_BRAIN");
        std::env::set_var("OCTOPUS_BRAIN_SCOUT_LLM_PREFIX", "OCTOPUS_SLOT_SCOUT");
        std::env::set_var("OCTOPUS_SLOT_SCOUT_MODEL", "scout-model");
        std::env::set_var("OCTOPUS_SLOT_SCOUT_BASE_URL", "https://scout.example/v1");
        std::env::remove_var("OCTOPUS_SLOT_SCOUT_API_KEY");
        std::env::set_var(
            "OCTOPUS_SLOT_SCOUT_CURL",
            curl.to_string_lossy().to_string(),
        );
        ...
        restore_env("OCTOPUS_BRAIN_LLM", old_brain);
        restore_env("OCTOPUS_BRAIN_LLM_PREFIX", old_brain_prefix);
        restore_env("OCTOPUS_BRAIN_SCOUT_LLM_PREFIX", old_scout_prefix);
        restore_env("OCTOPUS_SLOT_SCOUT_MODEL", old_model);
        restore_env("OCTOPUS_SLOT_SCOUT_BASE_URL", old_base_url);
        restore_env("OCTOPUS_SLOT_SCOUT_API_KEY", old_api_key);
        restore_env("OCTOPUS_SLOT_SCOUT_CURL", old_curl);
        ...
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("scout slot reply"));
        assert!(content.contains("scout-slot cognitive need"));
        let _ = fs::remove_dir_all(dir);
    }

    #[cfg(unix)]
    #[test]
    fn cli_brain_agenda_live_uses_slot_without_feed() {
        use std::os::unix::fs::PermissionsExt;

        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-agenda-live-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let curl = dir.join("agenda-curl.sh");
        fs::write(
            &curl,
            r#"#!/bin/sh
printf '%s' '{"choices":[{"message":{"content":"{\"summary\":\"agenda slot reply\",\"observations\":[\"slot observation\"],\"questions\":[\"slot question\"],\"options\":[\"slot option\"],\"risks\":[\"slot risk\"],\"needs\":[{\"kind\":\"verify\",\"query\":\"agenda-slot cognitive need\"}]}"}}]}'
"#,
        )
        .unwrap();
        let mut permissions = fs::metadata(&curl).unwrap().permissions();
        ...
        let old_model = std::env::var("OCTOPUS_SLOT_AGENDA_MODEL").ok();
        let old_base_url = std::env::var("OCTOPUS_SLOT_AGENDA_BASE_URL").ok();
        let old_api_key = std::env::var("OCTOPUS_SLOT_AGENDA_API_KEY").ok();
        let old_curl = std::env::var("OCTOPUS_SLOT_AGENDA_CURL").ok();
        std::env::set_var("OCTOPUS_BRAIN_LLM", "1");
        std::env::set_var("OCTOPUS_BRAIN_LLM_PREFIX", "OCTOPUS_UNUSED_BRAIN");
        std::env::set_var("OCTOPUS_BRAIN_AGENDA_LLM_PREFIX", "OCTOPUS_SLOT_AGENDA");
        std::env::set_var("OCTOPUS_SLOT_AGENDA_MODEL", "agenda-model");
        std::env::set_var("OCTOPUS_SLOT_AGENDA_BASE_URL", "https://agenda.example/v1");
        std::env::remove_var("OCTOPUS_SLOT_AGENDA_API_KEY");
        std::env::set_var(
            "OCTOPUS_SLOT_AGENDA_CURL",
            curl.to_string_lossy().to_string(),
        );
        ...
        restore_env("OCTOPUS_SLOT_AGENDA_MODEL", old_model);
        restore_env("OCTOPUS_SLOT_AGENDA_BASE_URL", old_base_url);
        restore_env("OCTOPUS_SLOT_AGENDA_API_KEY", old_api_key);
        restore_env("OCTOPUS_SLOT_AGENDA_CURL", old_curl);
        ...
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("agenda slot reply"));
        assert!(content.contains("agenda-slot cognitive need"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_reflect_session_writes_external_chat_artifacts_without_feed() {
        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-reflect-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        let reply = fs::read_to_string(session.join("REPLY.json")).unwrap();
        let command = fs::read_to_string(session.join("COMMANDS.sh")).unwrap();
        assert!(prompt.contains("mode: reflect"));
        assert!(messages.contains("reflection layer"));
        assert!(messages.contains("Goal + Mem + Need + Feed"));
        assert!(reply.contains("\"goal_state\""));
        assert!(reply.contains("\"gaps\""));
        assert!(command.contains("brain --reflect --apply"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_reflect_apply_saves_clean_needs_without_feed() {
        let _env = env_guard();
        let dir = std::env::temp_dir().join(format!(
            "octopus-brain-reflect-apply-{}",
            std::process::id()
        ));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        ...
        run(vec![
            ...
            "--reflect".to_string(),
            "--apply-json".to_string(),
            "{\"summary\":\"external reflection\",\"goal_state\":\"partial\",\"evidence\":[\"goal exists\"],\"gaps\":[\"evidence missing\"],\"questions\":[\"what proof matters?\"],\"needs\":[{\"kind\":\"verify\",\"query\":\"whether goal evidence is enough\"},{\"kind\":\"execute\",\"query\":\"cargo test -p octopus-core\"}]}".to_string(),
            "--save".to_string(),
            "reflect".to_string(),
            "goal".to_string(),
        ])
        ...
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("external reflection"));
        assert!(content.contains("whether goal evidence is enough"));
        assert!(!content.contains("cargo test"));
        let _ = fs::remove_dir_all(dir);
    }
    ...
    fn cli_brain_align_checks_goal_without_feed() {
        let _env = env_guard();
        let path = std::env::temp_dir().join(format!(
            "octopus-brain-align-state-{}.json",
            std::process::id()
        ));
        let state = path.to_string_lossy().to_string();
        let _ = fs::remove_file(&path);
    ...
    #[test]
    fn cli_brain_align_session_writes_external_chat_artifacts_without_feed() {
        let _env = env_guard();
        let dir = std::env::temp_dir().join(format!("octopus-brain-align-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        let reply = fs::read_to_string(session.join("REPLY.json")).unwrap();
        let command = fs::read_to_string(session.join("COMMANDS.sh")).unwrap();
        assert!(prompt.contains("mode: align"));
        assert!(messages.contains("alignment layer"));
        assert!(messages.contains("Goal + Mem + Need + Feed"));
        assert!(reply.contains("\"goal_state\""));
        assert!(reply.contains("alignment gap"));
        assert!(command.contains("brain --align --apply"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_align_apply_saves_clean_needs_without_feed() {
        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-align-apply-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        run(vec![
            ...
            "--align".to_string(),
            "--apply-json".to_string(),
            "{\"summary\":\"external alignment\",\"goal_state\":\"partial\",\"evidence\":[\"goal exists\"],\"gaps\":[\"constraint unclear\"],\"questions\":[\"what goal boundary matters?\"],\"needs\":[{\"kind\":\"verify\",\"query\":\"whether current Need follows the Goal\"},{\"kind\":\"execute\",\"query\":\"cargo test -p octopus-core\"}]}".to_string(),
            "--save".to_string(),
            "align".to_string(),
            "goal".to_string(),
        ])
        ...
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("external alignment"));
        assert!(content.contains("whether current Need follows the Goal"));
        assert!(!content.contains("cargo test"));
        let _ = fs::remove_dir_all(dir);
    }
    ...
    #[cfg(unix)]
    #[test]
    fn cli_brain_reflect_live_uses_slot_without_feed() {
        use std::os::unix::fs::PermissionsExt;

        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-reflect-live-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let curl = dir.join("reflect-curl.sh");
        fs::write(
            &curl,
            r#"#!/bin/sh
printf '%s' '{"choices":[{"message":{"content":"{\"summary\":\"reflect slot reply\",\"goal_state\":\"partial\",\"evidence\":[\"slot evidence\"],\"gaps\":[\"slot gap\"],\"questions\":[\"slot question\"],\"needs\":[{\"kind\":\"verify\",\"query\":\"reflect-slot cognitive need\"}]}"}}]}'
"#,
        )
        .unwrap();
        let mut permissions = fs::metadata(&curl).unwrap().permissions();
        ...
        let old_brain = std::env::var("OCTOPUS_BRAIN_LLM").ok();
        let old_brain_prefix = std::env::var("OCTOPUS_BRAIN_LLM_PREFIX").ok();
        let old_reflect_prefix = std::env::var("OCTOPUS_BRAIN_REFLECT_LLM_PREFIX").ok();
        let old_model = std::env::var("OCTOPUS_SLOT_REFLECT_MODEL").ok();
        let old_base_url = std::env::var("OCTOPUS_SLOT_REFLECT_BASE_URL").ok();
        let old_api_key = std::env::var("OCTOPUS_SLOT_REFLECT_API_KEY").ok();
        let old_curl = std::env::var("OCTOPUS_SLOT_REFLECT_CURL").ok();
        std::env::set_var("OCTOPUS_BRAIN_LLM", "1");
        std::env::set_var("OCTOPUS_BRAIN_LLM_PREFIX", "OCTOPUS_UNUSED_BRAIN");
        std::env::set_var("OCTOPUS_BRAIN_REFLECT_LLM_PREFIX", "OCTOPUS_SLOT_REFLECT");
        std::env::set_var("OCTOPUS_SLOT_REFLECT_MODEL", "reflect-model");
        std::env::set_var(
            "OCTOPUS_SLOT_REFLECT_BASE_URL",
            "https://reflect.example/v1",
        );
        std::env::remove_var("OCTOPUS_SLOT_REFLECT_API_KEY");
        std::env::set_var(
            "OCTOPUS_SLOT_REFLECT_CURL",
            curl.to_string_lossy().to_string(),
        );
        ...
        restore_env("OCTOPUS_BRAIN_LLM", old_brain);
        restore_env("OCTOPUS_BRAIN_LLM_PREFIX", old_brain_prefix);
        restore_env("OCTOPUS_BRAIN_REFLECT_LLM_PREFIX", old_reflect_prefix);
        restore_env("OCTOPUS_SLOT_REFLECT_MODEL", old_model);
        restore_env("OCTOPUS_SLOT_REFLECT_BASE_URL", old_base_url);
        restore_env("OCTOPUS_SLOT_REFLECT_API_KEY", old_api_key);
        restore_env("OCTOPUS_SLOT_REFLECT_CURL", old_curl);
        ...
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("reflect slot reply"));
        assert!(content.contains("reflect-slot cognitive need"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_memory_session_writes_external_chat_artifacts_without_feed() {
        let _env = env_guard();
        let dir = std::env::temp_dir().join(format!("octopus-brain-memory-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        let reply = fs::read_to_string(session.join("REPLY.json")).unwrap();
        let command = fs::read_to_string(session.join("COMMANDS.sh")).unwrap();
        assert!(prompt.contains("mode: memory"));
        assert!(messages.contains("memory layer"));
        assert!(messages.contains("Goal + Mem + Need + Feed"));
        assert!(reply.contains("\"remember\""));
        assert!(command.contains("brain --memory --apply"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_intent_session_writes_external_chat_artifacts_without_feed() {
        let _env = env_guard();
        let dir = std::env::temp_dir().join(format!("octopus-brain-intent-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        let reply = fs::read_to_string(session.join("REPLY.json")).unwrap();
        let command = fs::read_to_string(session.join("COMMANDS.sh")).unwrap();
        assert!(prompt.contains("mode: intent"));
        assert!(messages.contains("intent layer"));
        assert!(messages.contains("Goal + Mem + Need + Feed"));
        assert!(reply.contains("\"compare\""));
        assert!(command.contains("brain --intent --apply"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_brief_session_writes_external_chat_artifacts_without_feed() {
        let _env = env_guard();
        let dir = std::env::temp_dir().join(format!("octopus-brain-brief-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        let reply = fs::read_to_string(session.join("REPLY.json")).unwrap();
        let command = fs::read_to_string(session.join("COMMANDS.sh")).unwrap();
        assert!(prompt.contains("mode: brief"));
        assert!(messages.contains("brief layer"));
        assert!(messages.contains("Goal + Mem + Need + Feed"));
        assert!(reply.contains("\"compare\""));
        assert!(command.contains("brain --brief --apply"));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn cli_brain_memory_apply_saves_clean_needs_without_feed() {
        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-memory-apply-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        ...
        run(vec![
            ...
            "--memory".to_string(),
            "--apply-json".to_string(),
            "{\"summary\":\"memory reply\",\"needs\":[{\"kind\":\"remember\",\"query\":\"durable goal context\"},{\"kind\":\"execute\",\"query\":\"cargo test -p octopus-core\"}]}".to_string(),
            "--save".to_string(),
            "compress".to_string(),
            "memory".to_string(),
        ])
        ...
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("memory reply"));
        assert!(content.contains("durable goal context"));
        assert!(!content.contains("cargo test"));
        let _ = fs::remove_dir_all(dir);
    }
    ...
    #[cfg(unix)]
    #[test]
    fn cli_brain_memory_live_uses_slot_without_feed() {
        use std::os::unix::fs::PermissionsExt;

        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-memory-live-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let curl = dir.join("memory-curl.sh");
        fs::write(
            &curl,
            r#"#!/bin/sh
printf '%s' '{"choices":[{"message":{"content":"{\"summary\":\"memory slot reply\",\"needs\":[{\"kind\":\"remember\",\"query\":\"memory-slot durable context\"}]}"}}]}'
"#,
        )
        .unwrap();
        let mut permissions = fs::metadata(&curl).unwrap().permissions();
        ...
        let old_brain = std::env::var("OCTOPUS_BRAIN_LLM").ok();
        let old_brain_prefix = std::env::var("OCTOPUS_BRAIN_LLM_PREFIX").ok();
        let old_memory_prefix = std::env::var("OCTOPUS_BRAIN_MEMORY_LLM_PREFIX").ok();
        let old_model = std::env::var("OCTOPUS_SLOT_MEMORY_MODEL").ok();
        let old_base_url = std::env::var("OCTOPUS_SLOT_MEMORY_BASE_URL").ok();
        let old_api_key = std::env::var("OCTOPUS_SLOT_MEMORY_API_KEY").ok();
        let old_curl = std::env::var("OCTOPUS_SLOT_MEMORY_CURL").ok();
        std::env::set_var("OCTOPUS_BRAIN_LLM", "1");
        std::env::set_var("OCTOPUS_BRAIN_LLM_PREFIX", "OCTOPUS_UNUSED_BRAIN");
        std::env::set_var("OCTOPUS_BRAIN_MEMORY_LLM_PREFIX", "OCTOPUS_SLOT_MEMORY");
        std::env::set_var("OCTOPUS_SLOT_MEMORY_MODEL", "memory-model");
        std::env::set_var("OCTOPUS_SLOT_MEMORY_BASE_URL", "https://memory.example/v1");
        std::env::remove_var("OCTOPUS_SLOT_MEMORY_API_KEY");
        std::env::set_var(
            "OCTOPUS_SLOT_MEMORY_CURL",
            curl.to_string_lossy().to_string(),
        );
        ...
        restore_env("OCTOPUS_BRAIN_LLM", old_brain);
        restore_env("OCTOPUS_BRAIN_LLM_PREFIX", old_brain_prefix);
        restore_env("OCTOPUS_BRAIN_MEMORY_LLM_PREFIX", old_memory_prefix);
        restore_env("OCTOPUS_SLOT_MEMORY_MODEL", old_model);
        restore_env("OCTOPUS_SLOT_MEMORY_BASE_URL", old_base_url);
        restore_env("OCTOPUS_SLOT_MEMORY_API_KEY", old_api_key);
        restore_env("OCTOPUS_SLOT_MEMORY_CURL", old_curl);
        ...
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("memory slot reply"));
        assert!(content.contains("memory-slot durable context"));
        let _ = fs::remove_dir_all(dir);
    }
    ...
    #[cfg(unix)]
    #[test]
    fn cli_brain_council_collects_multiple_model_drafts_without_feed() {
        use std::os::unix::fs::PermissionsExt;

        let _env = env_guard();
        let dir =
            std::env::temp_dir().join(format!("octopus-brain-council-{}", std::process::id()));
        let state_path = dir.join("state.json");
        let state = state_path.to_string_lossy().to_string();
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let curl_a = dir.join("council-a.sh");
        let curl_b = dir.join("council-b.sh");
        fs::write(
            &curl_a,
            r#"#!/bin/sh
printf '%s' '{"choices":[{"message":{"content":"{\"summary\":\"council a\",\"observations\":[\"evidence is thin\"],\"questions\":[\"which evidence matters?\"],\"options\":[\"verify progress\"],\"risks\":[\"implementation leakage\"],\"needs\":[{\"kind\":\"verify\",\"query\":\"whether evidence is enough\"},{\"kind\":\"execute\",\"query\":\"cargo test -p octopus-core\"}]}"}}]}'
"#,
        )
        .unwrap();
        fs::write(
            &curl_b,
            r#"#!/bin/sh
printf '%s' '{"choices":[{"message":{"content":"{\"summary\":\"council b\",\"observations\":[\"goal still needs comparison\"],\"questions\":[\"what changed recently?\"],\"options\":[\"compare recent Feed\"],\"risks\":[\"next Need may be broad\"],\"needs\":[{\"kind\":\"compare\",\"query\":\"recent Feed against the goal\"},{\"kind\":\"remember\",\"query\":\"cognitive diversity stayed useful\"}]}"}}]}'
"#,
        )
        .unwrap();
        ...
        let old_a_model = std::env::var("OCTOPUS_COUNCIL_A_MODEL").ok();
        let old_a_base_url = std::env::var("OCTOPUS_COUNCIL_A_BASE_URL").ok();
        let old_a_api_key = std::env::var("OCTOPUS_COUNCIL_A_API_KEY").ok();
        let old_a_curl = std::env::var("OCTOPUS_COUNCIL_A_CURL").ok();
        let old_b_model = std::env::var("OCTOPUS_COUNCIL_B_MODEL").ok();
        let old_b_base_url = std::env::var("OCTOPUS_COUNCIL_B_BASE_URL").ok();
        let old_b_api_key = std::env::var("OCTOPUS_COUNCIL_B_API_KEY").ok();
        let old_b_curl = std::env::var("OCTOPUS_COUNCIL_B_CURL").ok();
        std::env::set_var(
            "OCTOPUS_BRAIN_COUNCIL_LLM_PREFIXES",
            "OCTOPUS_COUNCIL_A,OCTOPUS_COUNCIL_B",
        );
        std::env::set_var("OCTOPUS_COUNCIL_A_MODEL", "council-a-model");
        std::env::set_var("OCTOPUS_COUNCIL_A_BASE_URL", "https://a.example/v1");
        std::env::remove_var("OCTOPUS_COUNCIL_A_API_KEY");
        std::env::set_var(
            "OCTOPUS_COUNCIL_A_CURL",
            curl_a.to_string_lossy().to_string(),
        );
        std::env::set_var("OCTOPUS_COUNCIL_B_MODEL", "council-b-model");
        std::env::set_var("OCTOPUS_COUNCIL_B_BASE_URL", "https://b.example/v1");
        std::env::remove_var("OCTOPUS_COUNCIL_B_API_KEY");
        std::env::set_var(
            "OCTOPUS_COUNCIL_B_CURL",
        ...
        restore_env("OCTOPUS_COUNCIL_A_MODEL", old_a_model);
        restore_env("OCTOPUS_COUNCIL_A_BASE_URL", old_a_base_url);
        restore_env("OCTOPUS_COUNCIL_A_API_KEY", old_a_api_key);
        restore_env("OCTOPUS_COUNCIL_A_CURL", old_a_curl);
        restore_env("OCTOPUS_COUNCIL_B_MODEL", old_b_model);
        restore_env("OCTOPUS_COUNCIL_B_BASE_URL", old_b_base_url);
        restore_env("OCTOPUS_COUNCIL_B_API_KEY", old_b_api_key);
        restore_env("OCTOPUS_COUNCIL_B_CURL", old_b_curl);
        ...
        assert!(content.contains("whether evidence is enough"));
        assert!(content.contains("recent Feed against the goal"));
        assert!(content.contains("cognitive diversity stayed useful"));
        assert!(!content.contains("cargo test"));
        let _ = fs::remove_dir_all(dir);
    }
...
"#,
    ...
    fn cli_catalog_and_env_commands_run() {
        ...
        assert!(usage().contains("repair verify [query]"));
    ...
    fn bridge_static_has_embedded_app_assets() {
        ...
        let (install_type, install_script) = bridge_static("/install.sh").unwrap();
    ...
    fn cli_repair_report_exposes_latest_repair_plan() {
        ...
        assert!(plan
            ...
            .ends_with("REPAIR_DRAFT_EFFECTIVENESS.md"));
        assert!(plan
            .repair_draft_effectiveness_json
            .ends_with("REPAIR_DRAFT_EFFECTIVENESS.json"));
        assert_eq!(plan.repair_draft_effectiveness_used_count, "0");
        assert_eq!(plan.repair_draft_effectiveness_satisfied_count, "0");
        assert_eq!(plan.repair_draft_effectiveness_failed_count, "0");
        assert_eq!(plan.repair_draft_effectiveness_success_rate, "0.00");
        assert!(plan
            .repair_draft_strategy
            .ends_with("REPAIR_DRAFT_STRATEGY.md"));
        assert!(plan
            .repair_draft_strategy_json
            .ends_with("REPAIR_DRAFT_STRATEGY.json"));
        assert_eq!(
            plan.repair_draft_strategy_status,
            "collect_provider_draft_outcomes"
        );
        assert!(plan
            .repair_draft_strategy_next_need_query
            .contains("collect reviewed provider repair draft outcomes"));
        assert!(plan
            .repair_draft_strategy_effectiveness
            .ends_with("REPAIR_DRAFT_STRATEGY_EFFECTIVENESS.md"));
        assert!(plan
            .repair_draft_strategy_effectiveness_json
            .ends_with("REPAIR_DRAFT_STRATEGY_EFFECTIVENESS.json"));
        assert_eq!(plan.repair_draft_strategy_effectiveness_used_count, "0");
        assert_eq!(
            plan.repair_draft_strategy_effectiveness_satisfied_count,
            "0"
        );
        assert_eq!(plan.repair_draft_strategy_effectiveness_failed_count, "0");
        assert_eq!(
            plan.repair_draft_strategy_effectiveness_success_rate,
            "0.00"
        );
        assert!(plan.repair_patch_draft.ends_with("REPAIR_PATCH_DRAFT.md"));
        assert!(plan
            .repair_patch_draft_json
            .ends_with("REPAIR_PATCH_DRAFT.json"));
        assert_eq!(plan.repair_patch_draft_status, "draft_unavailable");
        assert_eq!(plan.repair_patch_draft_draft_status, "disabled");
        assert_eq!(plan.repair_patch_draft_has_patch, "false");
        assert!(plan
            .repair_patch_draft_effectiveness
            .ends_with("REPAIR_PATCH_DRAFT_EFFECTIVENESS.md"));
        assert!(plan
            .repair_patch_draft_effectiveness_json
            .ends_with("REPAIR_PATCH_DRAFT_EFFECTIVENESS.json"));
        assert_eq!(plan.repair_patch_draft_effectiveness_used_count, "0");
        ...
        assert!(plan
            ...
            .ends_with("REPAIR_PATCH_APPLY_EFFECTIVENESS.md"));
        assert!(plan
            .repair_patch_apply_effectiveness_json
            .ends_with("REPAIR_PATCH_APPLY_EFFECTIVENESS.json"));
        assert_eq!(plan.repair_patch_apply_effectiveness_used_count, "0");
        assert_eq!(plan.repair_patch_apply_effectiveness_satisfied_count, "0");
        assert_eq!(plan.repair_patch_apply_effectiveness_failed_count, "0");
        assert_eq!(plan.repair_patch_apply_effectiveness_success_rate, "0.00");
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_APPLY_EFFECTIVENESS.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_APPLY_EFFECTIVENESS.json")));
        assert!(plan.repair_patch_verify.ends_with("REPAIR_PATCH_VERIFY.md"));
        assert!(plan
            .repair_patch_verify_json
            .ends_with("REPAIR_PATCH_VERIFY.json"));
        assert_eq!(plan.repair_patch_verify_status, "waiting_for_patch_apply");
        assert_eq!(plan.repair_patch_verify_passed, "false");
        assert!(plan
            .repair_patch_verify_command
            .contains("harness-repair-agent"));
        assert!(plan
            .repair_patch_verify_effectiveness
            .ends_with("REPAIR_PATCH_VERIFY_EFFECTIVENESS.md"));
        assert!(plan
            .repair_patch_verify_effectiveness_json
            .ends_with("REPAIR_PATCH_VERIFY_EFFECTIVENESS.json"));
        assert_eq!(plan.repair_patch_verify_effectiveness_used_count, "0");
        assert_eq!(plan.repair_patch_verify_effectiveness_satisfied_count, "0");
        assert_eq!(plan.repair_patch_verify_effectiveness_failed_count, "0");
        assert_eq!(plan.repair_patch_verify_effectiveness_success_rate, "0.00");
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_VERIFY_EFFECTIVENESS.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_VERIFY_EFFECTIVENESS.json")));
        assert!(plan
            .repair_patch_learning
            .ends_with("REPAIR_PATCH_LEARNING.md"));
        assert!(plan
            .repair_patch_learning_json
            .ends_with("REPAIR_PATCH_LEARNING.json"));
        assert_eq!(
            plan.repair_patch_learning_status,
            "collect_verified_patch_outcomes"
        );
        assert_eq!(plan.repair_patch_learning_used_count, "0");
        assert_eq!(plan.repair_patch_learning_verified_count, "0");
        assert_eq!(plan.repair_patch_learning_verified_success_rate, "0.00");
        assert!(plan
            .repair_patch_learning_next_need_query
            .contains("collect scored repair patch verification outcomes"));
        assert!(plan
            .repair_patch_learning_effectiveness
            .ends_with("REPAIR_PATCH_LEARNING_EFFECTIVENESS.md"));
        assert!(plan
            .repair_patch_learning_effectiveness_json
            .ends_with("REPAIR_PATCH_LEARNING_EFFECTIVENESS.json"));
        assert_eq!(plan.repair_patch_learning_effectiveness_used_count, "0");
        assert_eq!(
            plan.repair_patch_learning_effectiveness_satisfied_count,
            "0"
        );
        assert_eq!(plan.repair_patch_learning_effectiveness_failed_count, "0");
        assert_eq!(
        ...
        assert!(plan
            .repair_patch_strategy_json
            .ends_with("REPAIR_PATCH_STRATEGY.json"));
        assert_eq!(plan.repair_patch_strategy_status, "collect_patch_outcomes");
        assert!(plan
            .repair_patch_strategy_focus
            .contains("score repair outcomes"));
        assert!(plan
        ...
        assert!(plan
            ...
            .ends_with("REPAIR_COMMAND_EFFECTIVENESS.json"));
        assert_eq!(plan.repair_command_effectiveness_used_count, "0");
        ...
        assert!(plan
            ...
            .ends_with("REPAIR_COMMAND_STRATEGY_EFFECTIVENESS.md"));
        assert!(plan
            .repair_command_strategy_effectiveness_json
            .ends_with("REPAIR_COMMAND_STRATEGY_EFFECTIVENESS.json"));
        assert_eq!(plan.repair_command_strategy_effectiveness_used_count, "0");
        assert_eq!(
            plan.repair_command_strategy_effectiveness_satisfied_count,
            "0"
        );
        assert_eq!(plan.repair_command_strategy_effectiveness_failed_count, "0");
        assert_eq!(
        ...
        assert!(plan
            ...
            .ends_with("REPAIR_EFFECTIVENESS_ROLLUP.md"));
        assert!(plan
        ...
        assert_eq!(plan.repair_effectiveness_rollup_failed_count, "0");
        assert_eq!(plan.repair_effectiveness_rollup_success_rate, "0.00");
        assert!(plan
            .repair_effectiveness_rollup_next_need_query
            .contains("collect scored repair outcomes"));
        assert!(plan
            .repair_effectiveness_rollup_effectiveness
            .ends_with("REPAIR_EFFECTIVENESS_ROLLUP_EFFECTIVENESS.md"));
        assert!(plan
            .repair_effectiveness_rollup_effectiveness_json
            .ends_with("REPAIR_EFFECTIVENESS_ROLLUP_EFFECTIVENESS.json"));
        assert_eq!(
        ...
        assert!(plan
            ...
            .ends_with("REPAIR_QUEUE_BRIEF_ADAPTATION.md"));
        assert!(plan
            .repair_queue_brief_adaptation_json
            .ends_with("REPAIR_QUEUE_BRIEF_ADAPTATION.json"));
        assert_eq!(
            plan.repair_queue_brief_adaptation_status,
            "collect_queue_brief_outcomes"
        );
        assert_eq!(plan.repair_queue_brief_adaptation_queue_used_count, "0");
        assert_eq!(plan.repair_queue_brief_adaptation_queue_failed_count, "0");
        assert_eq!(
            plan.repair_queue_brief_adaptation_queue_success_rate,
            "0.00"
        );
        assert!(plan
        ...
        assert!(plan_json.contains("\"repair_draft_effectiveness\""));
        assert!(plan_json.contains("\"repair_draft_strategy\""));
        assert!(plan_json.contains("\"repair_draft_strategy_effectiveness\""));
        ...
        assert!(plan_json.contains("\"repair_queue_brief_adaptation\""));
        ...
        assert!(plan_json.contains("\"repair_patch_apply_effectiveness\""));
        assert!(plan_json.contains("\"repair_patch_verify\""));
        assert!(plan_json.contains("\"repair_patch_verify_effectiveness\""));
        ...
        let repair_effectiveness_rollup_json =
            fs::read_to_string(&plan.repair_effectiveness_rollup_json).unwrap();
        ...
        assert!(repair_effectiveness_rollup_effectiveness_json.contains(
        ...
        let repair_queue_brief_adaptation_json =
            fs::read_to_string(&plan.repair_queue_brief_adaptation_json).unwrap();
        assert!(repair_queue_brief_adaptation_json
            .contains("\"schema_version\": \"octopus-harness-repair-queue-brief-adaptation-v1\""));
        assert!(repair_queue_brief_adaptation_json
            .contains("\"status\": \"collect_queue_brief_outcomes\""));
        let draft_effectiveness_json =
            fs::read_to_string(&plan.repair_draft_effectiveness_json).unwrap();
        assert!(draft_effectiveness_json
            .contains("\"schema_version\": \"octopus-harness-repair-draft-effectiveness-v1\""));
        assert!(draft_effectiveness_json.contains("\"used_count\": 0"));
        let draft_strategy_json = fs::read_to_string(&plan.repair_draft_strategy_json).unwrap();
        assert!(draft_strategy_json
            .contains("\"schema_version\": \"octopus-harness-repair-draft-strategy-v1\""));
        assert!(draft_strategy_json.contains("\"status\": \"collect_provider_draft_outcomes\""));
        let draft_strategy_effectiveness_json =
            fs::read_to_string(&plan.repair_draft_strategy_effectiveness_json).unwrap();
        assert!(draft_strategy_effectiveness_json.contains(
            "\"schema_version\": \"octopus-harness-repair-draft-strategy-effectiveness-v1\""
        ));
        assert!(draft_strategy_effectiveness_json.contains("\"used_count\": 0"));
        ...
        assert!(patch_draft_json.contains("\"status\": \"draft_unavailable\""));
        let patch_draft_effectiveness_json =
            fs::read_to_string(&plan.repair_patch_draft_effectiveness_json).unwrap();
        assert!(patch_draft_effectiveness_json.contains(
        ...
        let patch_apply_effectiveness_json =
            fs::read_to_string(&plan.repair_patch_apply_effectiveness_json).unwrap();
        assert!(patch_apply_effectiveness_json.contains(
            "\"schema_version\": \"octopus-harness-repair-patch-apply-effectiveness-v1\""
        ));
        assert!(patch_apply_effectiveness_json.contains("\"used_count\": 0"));
        let patch_verify_json = fs::read_to_string(&plan.repair_patch_verify_json).unwrap();
        assert!(patch_verify_json
            .contains("\"schema_version\": \"octopus-harness-repair-patch-verify-v1\""));
        assert!(patch_verify_json.contains("\"status\": \"waiting_for_patch_apply\""));
        assert!(patch_verify_json.contains("\"passed\": false"));
        let patch_verify_effectiveness_json =
            fs::read_to_string(&plan.repair_patch_verify_effectiveness_json).unwrap();
        assert!(patch_verify_effectiveness_json.contains(
            "\"schema_version\": \"octopus-harness-repair-patch-verify-effectiveness-v1\""
        ));
        assert!(patch_verify_effectiveness_json.contains("\"used_count\": 0"));
        let patch_learning_json = fs::read_to_string(&plan.repair_patch_learning_json).unwrap();
        assert!(patch_learning_json
            .contains("\"schema_version\": \"octopus-harness-repair-patch-learning-v1\""));
        assert!(patch_learning_json.contains("\"used_count\": 0"));
        assert!(patch_learning_json.contains("\"verified_count\": 0"));
        let patch_learning_effectiveness_json =
            fs::read_to_string(&plan.repair_patch_learning_effectiveness_json).unwrap();
        assert!(patch_learning_effectiveness_json.contains(
            "\"schema_version\": \"octopus-harness-repair-patch-learning-effectiveness-v1\""
        ));
        assert!(patch_learning_effectiveness_json.contains("\"used_count\": 0"));
        let patch_strategy_json = fs::read_to_string(&plan.repair_patch_strategy_json).unwrap();
        ...
        let command_strategy_effectiveness_json =
            fs::read_to_string(&plan.repair_command_strategy_effectiveness_json).unwrap();
        assert!(command_strategy_effectiveness_json.contains(
        ...
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_EFFECTIVENESS_ROLLUP.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_EFFECTIVENESS_ROLLUP.json")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_EFFECTIVENESS_ROLLUP_EFFECTIVENESS.md")));
        assert!(report
            .next
            .iter()
        ...
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_QUEUE_BRIEF_ADAPTATION.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_QUEUE_BRIEF_ADAPTATION.json")));
        assert!(report
            .next
            .iter()
        ...
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_DRAFT_EFFECTIVENESS.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_DRAFT_EFFECTIVENESS.json")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_DRAFT_STRATEGY.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_DRAFT_STRATEGY.json")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_DRAFT_STRATEGY_EFFECTIVENESS.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_DRAFT_STRATEGY_EFFECTIVENESS.json")));
        assert!(report
            .next
            .iter()
        ...
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_DRAFT.json")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_DRAFT_EFFECTIVENESS.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_DRAFT_EFFECTIVENESS.json")));
        assert!(report
            .next
            .iter()
        ...
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_VERIFY.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_VERIFY.json")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_LEARNING.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_LEARNING_EFFECTIVENESS.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_LEARNING.json")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_LEARNING_EFFECTIVENESS.md")));
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_LEARNING_EFFECTIVENESS.json")));
        assert!(report
            .next
            .iter()
        ...
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_PATCH_STRATEGY.json")));
        assert!(report
            .next
            .iter()
        ...
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("REPAIR_COMMAND_STRATEGY_EFFECTIVENESS.md")));
        assert!(report
            .next
            .iter()
    ...
    fn cli_repair_report_uses_failed_rollup_adaptation() {
        ...
        assert_eq!(
            report
                ...
                .get("next_need_source")
                .map(String::as_str),
            Some("repair_queue_brief_adaptation")
        );
        assert!(report
            .feed
            .summary
            .contains("source=repair_queue_brief_adaptation"));
        assert!(report
            .feed
            .summary
            .contains("queue_brief_adaptation=repair_queue_selection"));
        let plan = report
            .repair_plan
        ...
        assert_eq!(
            plan.repair_queue_brief_adaptation_status,
            "repair_queue_selection"
        );
        assert_eq!(plan.repair_queue_brief_adaptation_queue_used_count, "1");
        assert_eq!(plan.repair_queue_brief_adaptation_queue_failed_count, "1");
        assert_eq!(
            plan.repair_queue_brief_adaptation_queue_success_rate,
            "0.00"
        );
        assert!(plan
            ...
            .contains("repair heartbeat repair queue source selection"));
        let queue_adaptation_json =
            fs::read_to_string(&plan.repair_queue_brief_adaptation_json).unwrap();
        assert!(queue_adaptation_json
            .contains("\"schema_version\": \"octopus-harness-repair-queue-brief-adaptation-v1\""));
        assert!(queue_adaptation_json.contains("\"status\": \"repair_queue_selection\""));
    ...
    #[cfg(unix)]
    #[test]
    fn cli_repair_report_extracts_provider_patch_draft() {
        use std::os::unix::fs::PermissionsExt;

        let _env = env_guard();
        let _cwd = CwdGuard::new();
        let dir = std::env::temp_dir().join(format!(
            "octopus-repair-provider-patch-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
...
"#,
        ...
        assert!(plan
            .repair_patch_strategy_next_need_query
            .contains("review checked repair patch"));
        assert_eq!(
            plan.repair_patch_learning_status,
            "collect_verified_patch_outcomes"
        );
        ...
        assert!(draft_strategy_json
            .contains("\"schema_version\": \"octopus-harness-repair-draft-strategy-v1\""));
        assert!(draft_strategy_json.contains("\"status\": \"review_generated_repair_draft\""));
        let patch_strategy_json = fs::read_to_string(&plan.repair_patch_strategy_json).unwrap();
        assert!(patch_strategy_json
            .contains("\"schema_version\": \"octopus-harness-repair-patch-strategy-v1\""));
...
"#,
        ...
        assert!(report
            .next
            .iter()
            .any(|command| command.contains("octopus repair verify")));
        assert!(report.feed.summary.contains("patch_apply=applied"));

        let mut restored = HarnessState::load(&state_path).unwrap();
        let verified =
            repair_patch_verify_report(&state_path, &mut restored, ".".to_string()).unwrap();
        restored.save(&state_path).unwrap();
        assert_eq!(verified.status, "passed");
        assert!(verified.passed);
        assert!(verified
            .command
            .contains("octopus check harness-repair-agent"));
        assert!(verified.check.as_ref().is_some_and(|check| check.passed));
        let verified_json = fs::read_to_string(&verified.verify).unwrap();
        assert!(verified_json.contains("\"status\": \"passed\""));
        assert!(verified_json.contains("\"passed\": true"));
        ...
        assert_eq!(plan.repair_patch_verify_status, "passed");
        assert_eq!(plan.repair_patch_verify_passed, "true");
        assert!(plan.score_command.contains("repair score"));
        assert!(report.feed.summary.contains("patch_verify=passed"));
    ...
    fn cli_repair_report_blocks_on_missing_adapter_core() {
        ...
        fs::write(
            session_dir.join("ADAPTER_CONTEXT.md"),
            "# Adapter Context\n\nstatus: `partial`\nmissing_core: `git`\nprovider_env: `missing`\n",
        )
        .unwrap();
        ...
        assert!(report.queued.iter().any(|item| item
            .need
            .query
            .contains("install missing core adapters: git")));
        assert!(!report
            .next
            .iter()
    ...
    fn cli_repair_report_blocks_on_missing_repair_draft_config() {
        ...
        assert_eq!(
            plan.repair_draft_strategy_status,
            "repair_provider_draft_config"
        );
        let draft_strategy_json = fs::read_to_string(&plan.repair_draft_strategy_json).unwrap();
        assert!(draft_strategy_json
            .contains("\"schema_version\": \"octopus-harness-repair-draft-strategy-v1\""));
        assert!(draft_strategy_json.contains("\"status\": \"repair_provider_draft_config\""));
        assert!(plan.grant_command.is_empty());
        assert!(plan.apply_command.is_empty());
        assert!(plan.score_command.is_empty());
        assert!(report.queued.iter().any(|item| item
...
"#,
    ...
    fn cli_provider_save_writes_env_file() {
        ...
        assert!(content.contains("export OCTOPUS_BRAIN_INTENT_LLM_PREFIX=OCTOPUS_TEST"));
        assert!(content.contains("export OCTOPUS_BRAIN_BRIEF_LLM_PREFIX=OCTOPUS_TEST"));
        assert!(content.contains("export OCTOPUS_BRAIN_EXPLORE_LLM_PREFIX=OCTOPUS_TEST"));
        assert!(content.contains("export OCTOPUS_BRAIN_CLARIFY_LLM_PREFIX=OCTOPUS_TEST"));
        assert!(content.contains("export OCTOPUS_BRAIN_AGENDA_LLM_PREFIX=OCTOPUS_TEST"));
        assert!(content.contains("export OCTOPUS_BRAIN_SCOUT_LLM_PREFIX=OCTOPUS_TEST"));
        assert!(content.contains("export OCTOPUS_BRAIN_DELIBERATE_LLM_PREFIX=OCTOPUS_TEST"));
        assert!(content.contains("export OCTOPUS_BRAIN_REFLECT_LLM_PREFIX=OCTOPUS_TEST"));
        assert!(content.contains("export OCTOPUS_BRAIN_ALIGN_LLM_PREFIX=OCTOPUS_TEST"));
        assert!(content.contains("export OCTOPUS_BRAIN_MEMORY_LLM_PREFIX=OCTOPUS_TEST"));
    ...
    fn provider_status_reports_all_llm_layers() {
        ...
        let old_brain_prefix = std::env::var("OCTOPUS_BRAIN_LLM_PREFIX").ok();
        let old_brain_intent_prefix = std::env::var("OCTOPUS_BRAIN_INTENT_LLM_PREFIX").ok();
        let old_brain_brief_prefix = std::env::var("OCTOPUS_BRAIN_BRIEF_LLM_PREFIX").ok();
        let old_brain_explore_prefix = std::env::var("OCTOPUS_BRAIN_EXPLORE_LLM_PREFIX").ok();
        let old_brain_clarify_prefix = std::env::var("OCTOPUS_BRAIN_CLARIFY_LLM_PREFIX").ok();
        let old_brain_agenda_prefix = std::env::var("OCTOPUS_BRAIN_AGENDA_LLM_PREFIX").ok();
        let old_brain_scout_prefix = std::env::var("OCTOPUS_BRAIN_SCOUT_LLM_PREFIX").ok();
        let old_brain_deliberate_prefix = std::env::var("OCTOPUS_BRAIN_DELIBERATE_LLM_PREFIX").ok();
        let old_brain_reflect_prefix = std::env::var("OCTOPUS_BRAIN_REFLECT_LLM_PREFIX").ok();
        let old_brain_align_prefix = std::env::var("OCTOPUS_BRAIN_ALIGN_LLM_PREFIX").ok();
        let old_brain_memory_prefix = std::env::var("OCTOPUS_BRAIN_MEMORY_LLM_PREFIX").ok();
        ...
        std::env::set_var("OCTOPUS_BRAIN_INTENT_LLM_PREFIX", "OCTOPUS_STATUS_TEST");
        std::env::set_var("OCTOPUS_BRAIN_BRIEF_LLM_PREFIX", "OCTOPUS_STATUS_TEST");
        std::env::set_var("OCTOPUS_BRAIN_EXPLORE_LLM_PREFIX", "OCTOPUS_STATUS_TEST");
        std::env::set_var("OCTOPUS_BRAIN_CLARIFY_LLM_PREFIX", "OCTOPUS_STATUS_TEST");
        std::env::set_var("OCTOPUS_BRAIN_AGENDA_LLM_PREFIX", "OCTOPUS_STATUS_TEST");
        std::env::set_var("OCTOPUS_BRAIN_SCOUT_LLM_PREFIX", "OCTOPUS_STATUS_TEST");
        std::env::set_var("OCTOPUS_BRAIN_DELIBERATE_LLM_PREFIX", "OCTOPUS_STATUS_TEST");
        std::env::set_var("OCTOPUS_BRAIN_REFLECT_LLM_PREFIX", "OCTOPUS_STATUS_TEST");
        std::env::set_var("OCTOPUS_BRAIN_ALIGN_LLM_PREFIX", "OCTOPUS_STATUS_TEST");
        std::env::set_var("OCTOPUS_BRAIN_MEMORY_LLM_PREFIX", "OCTOPUS_STATUS_TEST");
        ...
        assert!(report.layers.iter().any(|layer| {
            layer.layer == "clean_brain_intent"
                && layer.prefix == "OCTOPUS_STATUS_TEST"
                && layer.enabled
        }));
        assert!(report.layers.iter().any(|layer| {
            layer.layer == "clean_brain_brief"
                && layer.prefix == "OCTOPUS_STATUS_TEST"
                && layer.enabled
        }));
        assert!(report.layers.iter().any(|layer| {
            layer.layer == "clean_brain_clarify"
                && layer.prefix == "OCTOPUS_STATUS_TEST"
                && layer.enabled
        }));
        assert!(report.layers.iter().any(|layer| {
            layer.layer == "clean_brain_agenda"
                && layer.prefix == "OCTOPUS_STATUS_TEST"
                && layer.enabled
        }));
        assert!(report.layers.iter().any(|layer| {
            layer.layer == "clean_brain_scout"
                && layer.prefix == "OCTOPUS_STATUS_TEST"
                && layer.enabled
        }));
        assert!(report.layers.iter().any(|layer| {
        ...
        assert!(report.layers.iter().any(|layer| {
            layer.layer == "clean_brain_reflect"
                && layer.prefix == "OCTOPUS_STATUS_TEST"
                && layer.enabled
        }));
        assert!(report.layers.iter().any(|layer| {
            layer.layer == "clean_brain_align"
                && layer.prefix == "OCTOPUS_STATUS_TEST"
                && layer.enabled
        }));
        assert!(report.layers.iter().any(|layer| {
            layer.layer == "clean_brain_memory"
                && layer.prefix == "OCTOPUS_STATUS_TEST"
                && layer.enabled
        }));
        assert!(report.layers.iter().any(|layer| {
        ...
        restore_env("OCTOPUS_BRAIN_LLM_PREFIX", old_brain_prefix);
        restore_env("OCTOPUS_BRAIN_INTENT_LLM_PREFIX", old_brain_intent_prefix);
        restore_env("OCTOPUS_BRAIN_BRIEF_LLM_PREFIX", old_brain_brief_prefix);
        restore_env("OCTOPUS_BRAIN_EXPLORE_LLM_PREFIX", old_brain_explore_prefix);
        restore_env("OCTOPUS_BRAIN_CLARIFY_LLM_PREFIX", old_brain_clarify_prefix);
        restore_env("OCTOPUS_BRAIN_AGENDA_LLM_PREFIX", old_brain_agenda_prefix);
        restore_env("OCTOPUS_BRAIN_SCOUT_LLM_PREFIX", old_brain_scout_prefix);
        restore_env(
            "OCTOPUS_BRAIN_DELIBERATE_LLM_PREFIX",
            old_brain_deliberate_prefix,
        );
        restore_env("OCTOPUS_BRAIN_REFLECT_LLM_PREFIX", old_brain_reflect_prefix);
        restore_env("OCTOPUS_BRAIN_ALIGN_LLM_PREFIX", old_brain_align_prefix);
        restore_env("OCTOPUS_BRAIN_MEMORY_LLM_PREFIX", old_brain_memory_prefix);
        restore_env(
    ...
    fn benchmark_record_requires_five_minimal_cases() {
        ...
        let audit = check_benchmark_record(&path, head.clone()).unwrap();
        assert!(!audit.passed);
        assert!(
            audit
                .checks
    ...
    fn cli_status_and_doctor_commands_run() {
        ...
        assert!(!audit.passed);
        let filled = record
            .replace("- Date:", "- Date: 2026-06-27")
            .replace("- Tester:", "- Tester: local")
            .replace("- Machine:", "- Machine: mac")
            .replace("- OS:", "- OS: macOS")
            .replace("- Shell:", "- Shell: zsh")
...
"#,
    ...
    fn cli_clean_brain_model_slots_route_goal_and_explore_without_feed() {
        ...
        let intent_curl = dir.join("intent-curl.sh");
        let brief_curl = dir.join("brief-curl.sh");
        let align_curl = dir.join("align-curl.sh");
...
"#,
        ...
        fs::write(
            &intent_curl,
            r#"#!/bin/sh
printf '%s' '{"choices":[{"message":{"content":"{\"summary\":\"intent slot reply\",\"needs\":[{\"kind\":\"compare\",\"query\":\"intent-slot cognitive need\"}]}"}}]}'
"#,
        )
        .unwrap();
        fs::write(
            &brief_curl,
            r#"#!/bin/sh
printf '%s' '{"choices":[{"message":{"content":"{\"summary\":\"brief slot reply\",\"needs\":[{\"kind\":\"verify\",\"query\":\"brief-slot cognitive need\"}]}"}}]}'
"#,
        )
        .unwrap();
        fs::write(
            &align_curl,
            r#"#!/bin/sh
printf '%s' '{"choices":[{"message":{"content":"{\"summary\":\"align slot reply\",\"goal_state\":\"aligned\",\"evidence\":[\"align evidence\"],\"gaps\":[\"align gap\"],\"questions\":[\"align question\"],\"needs\":[{\"kind\":\"verify\",\"query\":\"align-slot cognitive need\"}]}"}}]}'
"#,
        )
        .unwrap();
        fs::write(
...
"#,
        ...
        fs::write(
            &deliberate_curl,
            r#"#!/bin/sh
printf '%s' '{"choices":[{"message":{"content":"{\"summary\":\"deliberate slot reply\",\"observations\":[\"slot observation\"],\"questions\":[\"slot question\"],\"options\":[\"slot option\"],\"risks\":[\"slot risk\"],\"needs\":[{\"kind\":\"compare\",\"query\":\"deliberate-slot cognitive need\"}]}"}}]}'
"#,
        )
        .unwrap();
        fs::write(
...
"#,
        ...
        let old_intent_prefix = std::env::var("OCTOPUS_BRAIN_INTENT_LLM_PREFIX").ok();
        let old_brief_prefix = std::env::var("OCTOPUS_BRAIN_BRIEF_LLM_PREFIX").ok();
        let old_align_prefix = std::env::var("OCTOPUS_BRAIN_ALIGN_LLM_PREFIX").ok();
        ...
        let old_intent_model = std::env::var("OCTOPUS_SLOT_INTENT_MODEL").ok();
        let old_intent_base_url = std::env::var("OCTOPUS_SLOT_INTENT_BASE_URL").ok();
        let old_intent_api_key = std::env::var("OCTOPUS_SLOT_INTENT_API_KEY").ok();
        let old_intent_curl = std::env::var("OCTOPUS_SLOT_INTENT_CURL").ok();
        let old_brief_model = std::env::var("OCTOPUS_SLOT_BRIEF_MODEL").ok();
        let old_brief_base_url = std::env::var("OCTOPUS_SLOT_BRIEF_BASE_URL").ok();
        let old_brief_api_key = std::env::var("OCTOPUS_SLOT_BRIEF_API_KEY").ok();
        let old_brief_curl = std::env::var("OCTOPUS_SLOT_BRIEF_CURL").ok();
        let old_align_model = std::env::var("OCTOPUS_SLOT_ALIGN_MODEL").ok();
        let old_align_base_url = std::env::var("OCTOPUS_SLOT_ALIGN_BASE_URL").ok();
        let old_align_api_key = std::env::var("OCTOPUS_SLOT_ALIGN_API_KEY").ok();
        let old_align_curl = std::env::var("OCTOPUS_SLOT_ALIGN_CURL").ok();
        ...
        let old_deliberate_model = std::env::var("OCTOPUS_SLOT_DELIBERATE_MODEL").ok();
        ...
        std::env::set_var("OCTOPUS_BRAIN_INTENT_LLM_PREFIX", "OCTOPUS_SLOT_INTENT");
        std::env::set_var("OCTOPUS_BRAIN_BRIEF_LLM_PREFIX", "OCTOPUS_SLOT_BRIEF");
        std::env::set_var("OCTOPUS_BRAIN_ALIGN_LLM_PREFIX", "OCTOPUS_SLOT_ALIGN");
        ...
        std::env::set_var("OCTOPUS_SLOT_INTENT_MODEL", "intent-model");
        std::env::set_var("OCTOPUS_SLOT_INTENT_BASE_URL", "https://intent.example/v1");
        std::env::remove_var("OCTOPUS_SLOT_INTENT_API_KEY");
        std::env::set_var(
            "OCTOPUS_SLOT_INTENT_CURL",
            intent_curl.to_string_lossy().to_string(),
        );
        std::env::set_var("OCTOPUS_SLOT_BRIEF_MODEL", "brief-model");
        std::env::set_var("OCTOPUS_SLOT_BRIEF_BASE_URL", "https://brief.example/v1");
        std::env::remove_var("OCTOPUS_SLOT_BRIEF_API_KEY");
        std::env::set_var(
            "OCTOPUS_SLOT_BRIEF_CURL",
            brief_curl.to_string_lossy().to_string(),
        );
        std::env::set_var("OCTOPUS_SLOT_ALIGN_MODEL", "align-model");
        std::env::set_var("OCTOPUS_SLOT_ALIGN_BASE_URL", "https://align.example/v1");
        std::env::remove_var("OCTOPUS_SLOT_ALIGN_API_KEY");
        std::env::set_var(
            "OCTOPUS_SLOT_ALIGN_CURL",
            align_curl.to_string_lossy().to_string(),
        );
        ...
        std::env::set_var("OCTOPUS_SLOT_DELIBERATE_MODEL", "deliberate-model");
        std::env::set_var(
            "OCTOPUS_SLOT_DELIBERATE_BASE_URL",
            "https://deliberate.example/v1",
        );
        std::env::remove_var("OCTOPUS_SLOT_DELIBERATE_API_KEY");
        std::env::set_var(
        ...
        restore_env("OCTOPUS_BRAIN_INTENT_LLM_PREFIX", old_intent_prefix);
        restore_env("OCTOPUS_BRAIN_BRIEF_LLM_PREFIX", old_brief_prefix);
        restore_env("OCTOPUS_BRAIN_ALIGN_LLM_PREFIX", old_align_prefix);
        ...
        restore_env("OCTOPUS_SLOT_INTENT_MODEL", old_intent_model);
        restore_env("OCTOPUS_SLOT_INTENT_BASE_URL", old_intent_base_url);
        restore_env("OCTOPUS_SLOT_INTENT_API_KEY", old_intent_api_key);
        restore_env("OCTOPUS_SLOT_INTENT_CURL", old_intent_curl);
        restore_env("OCTOPUS_SLOT_BRIEF_MODEL", old_brief_model);
        restore_env("OCTOPUS_SLOT_BRIEF_BASE_URL", old_brief_base_url);
        restore_env("OCTOPUS_SLOT_BRIEF_API_KEY", old_brief_api_key);
        restore_env("OCTOPUS_SLOT_BRIEF_CURL", old_brief_curl);
        restore_env("OCTOPUS_SLOT_ALIGN_MODEL", old_align_model);
        restore_env("OCTOPUS_SLOT_ALIGN_BASE_URL", old_align_base_url);
        restore_env("OCTOPUS_SLOT_ALIGN_API_KEY", old_align_api_key);
        restore_env("OCTOPUS_SLOT_ALIGN_CURL", old_align_curl);
        ...
        restore_env("OCTOPUS_SLOT_DELIBERATE_MODEL", old_deliberate_model);
        ...
        assert_eq!(restored.pending_need_queue_count(), 7);
        assert!(restored.feed_traces.is_empty());
        assert!(restored.routes.scores.is_empty());
        let content = fs::read_to_string(&state_path).unwrap();
        assert!(content.contains("goal slot reply"));
        assert!(content.contains("goal-slot cognitive state"));
        assert!(content.contains("intent slot reply"));
        assert!(content.contains("intent-slot cognitive need"));
        assert!(content.contains("brief slot reply"));
        assert!(content.contains("brief-slot cognitive need"));
        assert!(content.contains("align slot reply"));
        assert!(content.contains("align-slot cognitive need"));
...
"#,
    ...
    fn cli_repair_score_records_outcome() {
        ...
        seeded.feed_traces[0]
            .metadata
            .insert("llm_draft_status".to_string(), "generated".to_string());
        seeded.feed_traces[0]
            .metadata
            .insert("llm_prefix".to_string(), "OCTOPUS_TEST".to_string());
        seeded.feed_traces[0]
            .metadata
            .insert("llm_model".to_string(), "mock-repair-model".to_string());
        seeded.feed_traces[0].metadata.insert(
            "action_trace_repair_draft_strategy_status".to_string(),
            "review_generated_repair_draft".to_string(),
        );
        seeded.feed_traces[0].metadata.insert(
            "action_trace_repair_draft_strategy_focus".to_string(),
            "mock-repair-model".to_string(),
        );
        seeded.feed_traces[0].metadata.insert(
        ...
        seeded.feed_traces[0].metadata.insert(
            ...
            "review generated provider repair draft".to_string(),
        );
        seeded.feed_traces[0].metadata.insert(
            "action_trace_repair_patch_draft_status".to_string(),
            "patch_attached".to_string(),
        );
        seeded.feed_traces[0].metadata.insert(
        ...
        seeded.feed_traces[0].metadata.insert(
            ...
            "patch applied with harness write grant".to_string(),
        );
        seeded.feed_traces[0].metadata.insert(
        ...
        assert!(outcomes.contains("\"draft_status\":\"generated\""));
        assert!(outcomes.contains("\"draft_prefix\":\"OCTOPUS_TEST\""));
        assert!(outcomes.contains("\"draft_model\":\"mock-repair-model\""));
        assert!(outcomes.contains("\"repair_command_check\":\"octopus check"));
        assert!(outcomes.contains("\"repair_command_grant\":\"octopus oauth"));
        assert!(outcomes.contains("\"repair_command_apply\":\"octopus "));
        ...
        assert!(outcomes.contains(
            "\"action_trace_repair_draft_strategy_status\":\"review_generated_repair_draft\""
        ));
        assert!(
            outcomes.contains("\"action_trace_repair_draft_strategy_focus\":\"mock-repair-model\"")
        );
        assert!(outcomes.contains(
            "\"action_trace_repair_draft_strategy_learned_reuse\":\"mock-repair-model\""
        ));
        ...
        assert!(outcomes.contains("\"action_trace_repair_patch_apply_status\":\"applied\""));
        assert!(outcomes.contains("\"action_trace_repair_patch_apply_applied\":\"true\""));
        assert!(outcomes.contains("\"action_trace_repair_patch_apply_target\":\"swe-agent/read\""));
        assert!(outcomes.contains("\"action_trace_repair_patch_verify_status\":\"passed\""));
        assert!(outcomes.contains("\"action_trace_repair_patch_verify_passed\":\"true\""));
        assert!(outcomes.contains("\"action_trace_repair_patch_verify_target\":\"swe-agent/read\""));
        assert!(outcomes
            .contains("\"action_trace_repair_patch_verify_command\":\"octopus check swe-agent\""));
        assert!(outcomes
            .contains("\"action_trace_repair_patch_strategy_status\":\"review_provider_patch\""));
        assert!(outcomes
            .contains("\"action_trace_repair_patch_strategy_focus\":\"provider repair patch\""));
        ...
        assert!(outcome_markdown.contains("draft_prefix: `OCTOPUS_TEST`"));
        assert!(outcome_markdown.contains("draft_model: `mock-repair-model`"));
        assert!(outcome_markdown.contains("repair_command_check: `octopus check"));
        ...
        assert!(outcome_markdown.contains(
            "action_trace_repair_draft_strategy: status=`review_generated_repair_draft`"
        ));
        assert!(outcome_markdown
        ...
        assert!(outcome_markdown
            .contains("action_trace_repair_patch_apply: status=`applied` applied=`true`"));
        assert!(outcome_markdown
            .contains("action_trace_repair_patch_verify: status=`passed` passed=`true`"));
        assert!(outcome_markdown
            .contains("action_trace_repair_patch_strategy: status=`review_provider_patch`"));
        assert!(outcome_markdown
        ...
        let draft_effectiveness_found = workspace
            .join(".octopus/harness-repair")
            .read_dir()
            .unwrap()
            .filter_map(|entry| {
                let candidate = entry.ok()?.path().join("REPAIR_DRAFT_EFFECTIVENESS.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
                    .map(|content| {
                        content.contains(
                            ...
                            && content.contains("mock-repair-model")
                    })
                    .unwrap_or(false)
            });
        assert!(draft_effectiveness_found);
        let draft_strategy_found = workspace
            .join(".octopus/harness-repair")
            .read_dir()
            .unwrap()
            .filter_map(|entry| {
                let candidate = entry.ok()?.path().join("REPAIR_DRAFT_STRATEGY.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
                    .map(|content| {
                        ...
                        ) && content.contains("\"status\": \"reuse_effective_repair_draft\"")
                            && content.contains("review repair plan with effective provider draft")
                            && content.contains("mock-repair-model")
                    })
                    .unwrap_or(false)
            });
        assert!(draft_strategy_found);
        let draft_strategy_effectiveness_found = workspace
            .join(".octopus/harness-repair")
            .read_dir()
            .unwrap()
            .filter_map(|entry| {
                let candidate = entry
                    .ok()?
                    .path()
                    .join("REPAIR_DRAFT_STRATEGY_EFFECTIVENESS.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
                    .map(|content| {
                        content.contains(
                            ...
                            && content.contains("\"satisfied_count\": 1")
                            && content.contains("\"success_rate\": \"1.00\"")
                            && content.contains("review_generated_repair_draft")
                            && content.contains("mock-repair-model")
                    })
                    .unwrap_or(false)
            });
        assert!(draft_strategy_effectiveness_found);
        let patch_draft_effectiveness_found = workspace
            .join(".octopus/harness-repair")
            .read_dir()
            .unwrap()
            .filter_map(|entry| {
                let candidate = entry
                    .ok()?
                    .path()
                    .join("REPAIR_PATCH_DRAFT_EFFECTIVENESS.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
                    .map(|content| {
                        content.contains(
                            ...
                            && content.contains("\"satisfied_count\": 1")
                            && content.contains("\"success_rate\": \"1.00\"")
                            && content.contains("patch_attached")
        ...
        let patch_apply_effectiveness_found = workspace
            .join(".octopus/harness-repair")
            .read_dir()
            .unwrap()
            .filter_map(|entry| {
                let candidate = entry
                    .ok()?
                    .path()
                    .join("REPAIR_PATCH_APPLY_EFFECTIVENESS.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
                    .map(|content| {
                        content.contains(
                            ...
                            && content.contains("\"satisfied_count\": 1")
                            && content.contains("\"success_rate\": \"1.00\"")
                            && content.contains("apply=applied")
                            && content.contains("swe-agent/read")
                    })
                    .unwrap_or(false)
        });
        assert!(patch_apply_effectiveness_found);
        let patch_verify_effectiveness_found = workspace
            .join(".octopus/harness-repair")
            .read_dir()
            .unwrap()
            .filter_map(|entry| {
                let candidate = entry
                    .ok()?
                    .path()
                    .join("REPAIR_PATCH_VERIFY_EFFECTIVENESS.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
                    .map(|content| {
                        content.contains(
                            ...
                            && content.contains("\"satisfied_count\": 1")
                            && content.contains("\"success_rate\": \"1.00\"")
                            && content.contains("verify=passed")
                            && content.contains("octopus check swe-agent")
                            && content.contains("swe-agent/read")
                    })
                    .unwrap_or(false)
            });
        assert!(patch_verify_effectiveness_found);
        let patch_learning_found = workspace
            .join(".octopus/harness-repair")
            .read_dir()
            .unwrap()
            .filter_map(|entry| {
                let candidate = entry.ok()?.path().join("REPAIR_PATCH_LEARNING.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
                    .map(|content| {
                        ...
                        ) && content.contains("\"status\": \"reuse_verified_patch\"")
                            && content.contains("\"used_count\": 1")
                            && content.contains("\"verified_count\": 1")
                            && content.contains("\"verified_success_rate\": \"1.00\"")
                            && content.contains("verify=passed")
                            && content.contains("swe-agent/read")
                    })
                    .unwrap_or(false)
            });
        assert!(patch_learning_found);
        let after_learning_session = HarnessState::load(&path).unwrap();
        assert_eq!(
            after_learning_session.feed_traces[1]
                .metadata
                .get("repair_draft_strategy_status")
                .map(String::as_str),
            Some("reuse_effective_repair_draft")
        );
        assert_eq!(
            after_learning_session.feed_traces[1]
                .metadata
        ...
        assert_eq!(
            after_learning_session.feed_traces[1]
                .metadata
                .get("repair_effectiveness_rollup_effectiveness_used_count")
                .map(String::as_str),
            Some("1")
        );
        assert_eq!(
            after_learning_session.feed_traces[1]
                .metadata
                .get("repair_effectiveness_rollup_effectiveness_satisfied_count")
                .map(String::as_str),
            Some("1")
        );
        assert_eq!(
            after_learning_session.feed_traces[1]
                .metadata
                .get("repair_effectiveness_rollup_effectiveness_success_rate")
                .map(String::as_str),
            Some("1.00")
        );
        assert_eq!(
        ...
        let patch_learning_effectiveness_found = workspace
            ...
            .filter_map(|entry| {
                let candidate = entry
                    .ok()?
                    .path()
                    .join("REPAIR_PATCH_LEARNING_EFFECTIVENESS.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
                    .map(|content| {
                        content.contains(
                            ...
                            && content.contains("\"satisfied_count\": 1")
                            && content.contains("\"success_rate\": \"1.00\"")
                            && content.contains("reuse_verified_patch")
                    })
                    .unwrap_or(false)
            });
        assert!(patch_learning_effectiveness_found);
        let patch_strategy_found = workspace
            .join(".octopus/harness-repair")
            .read_dir()
            .unwrap()
            .filter_map(|entry| {
                let candidate = entry.ok()?.path().join("REPAIR_PATCH_STRATEGY.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
        ...
        let rollup_effectiveness_artifact_found = workspace
            .join(".octopus/harness-repair")
            .read_dir()
            .unwrap()
            .filter_map(|entry| {
                let candidate = entry
                    .ok()?
                    .path()
                    .join("REPAIR_EFFECTIVENESS_ROLLUP_EFFECTIVENESS.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
        ...
        let command_effectiveness_found = workspace
            ...
            .filter_map(|entry| {
                let candidate = entry
                    .ok()?
                    .path()
                    .join("REPAIR_COMMAND_EFFECTIVENESS.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
        ...
        let command_strategy_effectiveness_found = workspace
            .join(".octopus/harness-repair")
            .read_dir()
            .unwrap()
            .filter_map(|entry| {
                let candidate = entry
                    .ok()?
                    .path()
                    .join("REPAIR_COMMAND_STRATEGY_EFFECTIVENESS.json");
                candidate.exists().then_some(candidate)
            })
            .any(|path| {
                fs::read_to_string(path)
        ...
        assert!(strategy_report
            .feed
            .summary
            .contains("command_strategy_effectiveness=2"));
        assert!(strategy_report
            .feed
            .summary
            .contains("decision_effectiveness=2"));
        assert!(strategy_report
            .feed
            .summary
        ...
        assert!(strategy_report
            .feed
            .summary
            .contains("draft_strategy_effectiveness=2"));
        assert!(strategy_report
            .feed
            .summary
        ...
        assert!(strategy_report
            .feed
            .summary
            .contains("action_trace_effectiveness=2"));
        assert!(strategy_report
            .queued
            .iter()
