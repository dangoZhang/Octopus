...
struct DoctorReport {
    ...
    profile_registry: ProfileRegistryReport,
    llm: DoctorLlmReport,
    pet: DoctorPetReport,
...
#[derive(Debug, serde::Serialize)]
struct PreflightRecordCheckReport {
    path: String,
    current_head: Option<String>,
    passed: bool,
    checks: Vec<PreflightCheck>,
...
fn run(args: Vec<String>) -> Result<(), String> {
    ...
    match rest.first().map(String::as_str) {
        ...
        Some("brain") => {
            ...
            if council && session {
                return Err(
                    "brain --council writes a direct council report, not a session".to_string(),
                );
            }
            if council && has_apply_payload {
                return Err(
                    "brain --council collects live drafts and does not accept --apply".to_string(),
                );
            }
            ...
            if council {
                ...
                if refine_goal {
                    ...
                    let draft = parse_brain_reply::<BrainReflectionDraft>(
                        &payload,
                        "clean-brain reflection",
                    )?;
                    ...
                    let draft =
                        parse_brain_reply::<BrainExploreDraft>(&payload, "clean-brain memory")?;
                    ...
                    let draft =
                        parse_brain_reply::<BrainExploreDraft>(&payload, "clean-brain brief")?;
                    ...
                    let draft =
                        parse_brain_reply::<BrainExploreDraft>(&payload, "clean-brain intent")?;
                    ...
                    let draft =
                        parse_brain_reply::<BrainDeliberationDraft>(&payload, "clean-brain scout")?;
...
fn check_preflight_record(path: &Path) -> Result<PreflightRecordCheckReport, String> {
    let current_head = git_short_head();
    let content = fs::read_to_string(path).unwrap_or_default();
    let file_exists = path.exists();
...
fn print_preflight_record_check(report: &PreflightRecordCheckReport, language: Language) {
    match language {
        Language::En => {
...
fn clean_brain_intent_llm_prefix() -> String {
    clean_brain_slot_llm_prefix("OCTOPUS_BRAIN_INTENT_LLM_PREFIX")
}

fn clean_brain_brief_llm_prefix() -> String {
...
fn clean_brain_council_llm_prefixes() -> Vec<String> {
