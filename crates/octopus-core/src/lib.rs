...
use llm_provider::plan_from_llm_content;
pub use llm_provider::{
    chat_client_factory_from_openai_config, ChatClient, ChatClientFactory, ChatMessage,
    ChatPlanner, ChatResponse, ChatRole, CodexCliChatClient, CodexCliConfig,
    OpenAiCompatibleChatClient, OpenAiCompatibleConfig, OpenAiCompatibleTuning,
};
pub(crate) use manifest_catalog::installed_tentacle_from_manifest;
pub use manifest_catalog::{
    default_tentacle_profiles, embedded_profile_registry_json, inspect_tentacle_manifests,
    load_tentacle_manifests, load_tentacle_profiles_from_path, InstalledTentacle, InstalledTool,
    LoadedTentacleManifest, ManifestBrain, ManifestSkill, ManifestTool, SkillManifest,
    TentacleBrain, TentacleManifest, TentacleManifestReport, TentacleProfile, ToolImplementation,
    ToolMetadata, ToolPermission,
};
#[cfg(test)]
pub(crate) use manifest_runtime::tool_result_from_json;
pub(crate) use manifest_runtime::{
    active_grant_for_tool, installed_tentacle_tools, tool_permission_text, InstalledToolRef,
};
pub use manifest_runtime::{
    ManifestTentacle, TentacleThinkingPlan, TentacleToolAction, TentacleToolCandidate,
};
...
"#,
    ...
    fn field_mini_task_tentacle_supports_declared_needs_with_llm_available() {
        ...
        let structured_plan = structured_tentacle
            .thinking_plan(&task_need)
            .expect("structured field mini task plan");
        assert_eq!(structured_plan.plan_source, "structured-need");
        assert_eq!(structured_plan.selected_tool.id, "run_field_mini_task");
