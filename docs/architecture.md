...
`OpenAiCompatibleChatClient` in Rust and `OpenAICompatibleLLM` in Python adapt any chat-completions-compatible provider through `OCTOPUS_LLM_MODEL`, `OCTOPUS_LLM_BASE_URL`, and `OCTOPUS_LLM_API_KEY`. The Rust adapter keeps the kernel light by using a `curl` runtime adapter. Set `OCTOPUS_CHAT_LLM=1` to let chat refine the active goal and suggest cognitive needs without choosing tools. Set `OCTOPUS_LLM_MANIFEST=1` to let installed manifest tentacles ask the provider to pick tools before execution. Set `OCTOPUS_LLM_EVOLVE=1` to let harness evolution candidates come from the provider. `OCTOPUS_CHAT_LLM_PREFIX`, `OCTOPUS_MANIFEST_LLM_PREFIX`, and `OCTOPUS_EVOLVE_LLM_PREFIX` can route those layers to different provider env groups.
