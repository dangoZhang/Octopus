...
`bridge [addr]` serves the native HTML app and exposes `/api/run` plus `/api/stream` for local Octopus subcommands so the GUI can execute init, provider env/status/check, install, check, chat, think, Need, pet, doctor, and beat flows without a shell. Bridge overlays `.octopus/llm.env` onto child Octopus commands. The app can generate or save provider env, read `--json provider status`, run live provider checks, read `--json install` reports, run seed-tentacle checks, show harness-beat evolution recommendations with an apply-plan preview, score the recommendation after review, and grant local `octopus tool:*` scopes; bridge does not allow GitHub OAuth, direct patch apply, provider env writes outside the default path, or PR publishing.
