...
`bridge [addr]` serves the native HTML app and exposes `/api/run` plus `/api/stream` for local Octopus subcommands so the GUI can execute init, provider status/check, install, check, chat, think, Need, pet, doctor, and beat flows without a shell. The app can read `--json provider status`, run live provider checks, read `--json install` reports, run seed-tentacle checks, show harness-beat evolution recommendations with an apply-plan preview, score the recommendation after review, and grant local `octopus tool:*` scopes; bridge does not allow GitHub OAuth, direct patch apply, or PR publishing.
