...
`OpenAiCompatibleChatClient` in Rust and `OpenAICompatibleLLM` in Python adapt any chat-completions-compatible provider. `providers` lists built-in OpenAI-compatible profiles; `provider <name> [prefix]` emits shell env for chat goal refinement, manifest tool planning, and harness evolution; `provider status` shows all three LLM layers without a network call; `provider check [prefix]` validates the live endpoint with the same adapter. The Rust adapter keeps the kernel light by using a `curl` runtime adapter. `OCTOPUS_CHAT_LLM_PREFIX`, `OCTOPUS_MANIFEST_LLM_PREFIX`, and `OCTOPUS_EVOLVE_LLM_PREFIX` can still route those layers to different provider env groups.
...
`bridge [addr]` serves the native HTML app and exposes `/api/run` plus `/api/stream` for local Octopus subcommands so the GUI can execute init, install, check, chat, think, Need, pet, doctor, and beat flows without a shell. The app can read `--json install` reports, run seed-tentacle checks, and grant local `octopus tool:*` scopes; bridge does not allow GitHub OAuth or PR publishing.
...
`evolve <tentacle> <objective>` writes an auditable draft under `.octopus/evolution/` listing the surfaces, prompt, metadata, code files, patch candidates, per-candidate patch drafts, checks, constraints, scored outcomes, and recent Feed traces that can guide safe change. Runtime-code candidates use recent traces to target the concrete tool entrypoint that actually produced Feed, instead of staying at a broad `tools/*` surface. With `OCTOPUS_LLM_EVOLVE=1`, candidates are generated by the provider from harness surfaces, outcomes, and trace data. `evolve recommend <tentacle>` scores candidates from previous outcomes and writes the next apply plan. `evolve apply <tentacle> <candidate>` writes an apply plan; with `harness:write`, it also emits a reviewable `.patch` file without applying it. `evolve score <tentacle> <candidate> <status>` stores the outcome in harness state so later proposals can see what worked.
