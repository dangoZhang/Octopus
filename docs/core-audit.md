...
- Local implementation logic that chooses work for the clean brain or leaks tool/API/file burden into brain context.
...
## Audit Pass 1: Hidden Tool-Thinking Removal
...
- Removed code: `ChatPlanner` no longer stores or invokes hidden rule execution after LLM failure or invalid Plan JSON.
- Removed exception: the old explicit rule-planning test/example path is gone; tool-side planning now needs a model-backed planner or returns no plan.
...
- Moved harness units: the bundled seed materializer moved to `bundled_harness.rs`; editable source-of-truth manifests, tools, and profile data remain under `tentacles/`.
- Remaining candidates: continue shrinking `main.rs`, review historical docs/log residue, and remove any stale code that chooses Feed implementation for the clean brain.
...
- Remaining candidates: continue shrinking `main.rs`, review historical docs/log residue, and remove any stale local code that chooses Feed implementation for the clean brain.
...
- Completed passes: hidden planner removal, explicit seed tool contracts, bundled harness boundary, download manifest boundary, shared shell display quoting, pixel pet boundary, profile registry boundary, and boundary report alignment.
