...
- `harness-repair-agent` can read heartbeat/evolution/state signals, probe adapter readiness, write `.octopus/harness-repair/SESSION.*`, and return a compact repair Feed with next grant/apply/score steps.
...
- Added a seed `harness-repair-agent` so heartbeat/evolution artifacts can be inspected by an LLM-backed tentacle as normal Need-to-Feed work, including a reviewable repair-session artifact.
