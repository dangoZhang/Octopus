...
- `harness-repair-agent` can read heartbeat/evolution/state signals, probe adapter readiness, write `.octopus/harness-repair/SESSION.*`, `PROMPT.md`, `NEXT_NEED.json`, and `COMMANDS.sh`, then return a compact repair Feed with next grant/apply/score steps.
...
- Added a seed `harness-repair-agent` so heartbeat/evolution artifacts can be inspected by an LLM-backed tentacle as normal Need-to-Feed work, including reviewable and LLM-ready repair-session artifacts.
