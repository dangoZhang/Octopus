...
- Self-iteration plans now expose a structured `next_action`: report-only plans point to the GitHub grant, authorized plans point to the dry-run PR path, and CLI/JSON output preserves the active `--state` path.
- Three beats exist: heartbeat, memory compaction, and harness adaptation. Harness evolution artifacts now require a real LLM planner; no local rule path writes evolution candidates.
...
- Native desktop pet now keeps Need bubbles and action bubbles independent: a fast-consumed Need can still appear briefly, code-as-harness action bubbles can appear during action or just after Feed returns, and invalid `--workers` values fail in both the Rust launcher and the standalone Swift observer instead of silently launching the wrong observer count.
- Native desktop pet now gives the left Need bubble a subtle state-colored pulse, so Need appears as live observation while the bubble position and text layout stay stable.
- Native desktop pet now maps each worker window to its own queued Need, field label, worker status color, and work/Feed bubbles, so multiple observer pets show parallel field workers instead of repeating the first worker's generic goal or the last global Feed state.
- Native desktop pet now keeps global action bubbles on the primary observer unless a worker window has fresh activity, so idle peer-field observers do not all animate from one global event.
- Native desktop pet now uses per-worker status color only while that worker is fresh, so stale Feed or blocked results do not masquerade as current activity.
- Native desktop pet now orders observer windows to the front without forcing app activation, so the pet appears without stealing focus from the user's current work.
- Native desktop pet now observes the peer field pool from `.octopus/state.json` when no active worker Need or fresh worker activity is visible: state saves include a derived `field_pool` snapshot, the pet falls back to legacy traces for older state files, puts the active field first, shows `active none` when all peer fields are complete, preserves the default one-pet view unless `--workers n` is requested, and adds the pool summary plus worker policy to the click-open Goal sheet without sending control commands.
- Native desktop pet now keeps a fresh worker's left Need bubble visible from a compact, command-free field/task label after the queued Need has already moved into Feed, then lets it disappear when the worker update is no longer fresh.
- Native desktop pet legacy observation now reads the latest parallel run `worker_policy` before falling back to the built-in policy, so old state files still show workers as peer-field execution slots.
- Parallel worker records now carry `updated_at_secs`; the desktop pet uses it to keep worker colors visible while making per-worker work/Feed bubbles disappear after the fresh observation window.
- Desktop pet freshness now uses one shared `observationFreshSeconds` window for event bubbles and worker bubbles, so Need and Feed animations cannot drift into different lifetimes.
- Worker Need bubbles now use the worker's own freshness window as well as pending Need state, so a fast-consumed parallel Need can still be visible on its matching desktop pet.
- `pet desktop --workers n` now keeps at least `n` observer windows while waiting for worker records and expands to the active parallel run count when more workers exist, so manual observer slots do not collapse behind stale one-worker state.
- The native desktop pet only expands to the latest parallel run's worker count while that run has pending worker Needs or fresh worker activity; stale run history falls back to the requested observer count and the field-pool overview.
- Extra desktop observer windows without worker records render as waiting `observer-slot` pets instead of copying another worker's Need, action, or Feed state.
- Native desktop pet source now has a regression check for ephemeral Need/action bubbles, so state observation does not drift back into sticky or missing bubbles.
- `pet desktop` now launches the observer before requiring a harness state file, so a fresh checkout can open the requested observer slots and wait for Octopus state instead of failing before work starts.
- `preflight` now typechecks the embedded Swift desktop pet source on macOS, so the native observer is part of release readiness without launching UI during checks.
...
- Local app status now reads the same latest parallel run fields, so the app can show requested workers, active workers, and candidate fields without exposing extra controls.
- `octopus status` now prints a `parallel_run` observation line with requested worker slots, active worker slots, candidate pool, and worker policy.
- Field-pack loading has started as `0.1.x` foundation toward `0.2.0`: Rust loads `field-packs/`, `octopus fields` can inspect and match packs, Need context receives `field_pack`, `field_mini_task`, and expected Feed, Feed traces carry the selected field, and `evolve parallel --workers 1 --open` now opens one worker slot from the required v0.2 peer field goals while opening the desktop observer. Multi-field objectives define a candidate pool; field status and recent-run fairness choose worker slots, `--workers n` enqueues one Need per worker slot, `evolve parallel` prepares `field-mini-task` in a fresh state, the queued-Need Feed path repairs broken installed `field-mini-task` sources before Feed, and workers now record the actual trace/verifier/status after Feed returns.
- Field docs now state the eight `v0.2.0` domains as one parallel Goal pool: math, search, code, SWE, research, computer-use, IB work, and robotics are peer adaptation surfaces. `--workers n` is execution capacity only, and `mini-*` adds depth inside each field.
- `fields summary` now prints the same pool policy, and stale misnamed field-report compatibility aliases were removed from the Rust report type so product language and core contracts use active peer slots.
- `evolve parallel` now names workers as execution slots from the peer field pool in both English and Chinese output.
- Parallel evolution runs now persist `worker_policy` and expose it in JSON, so outer tools can show workers as execution capacity without parsing summary prose.
- Run state, status `field_pool`, and product report now share one Rust `parallel_worker_policy()` contract: workers are execution slots from the peer field pool; fields stay peer.
- Parallel evolution runs now also persist `field_pool_size` and `field_pool_policy`, so observer tools can distinguish the peer field pool from the number of active worker execution slots.
...
- When all current peer-field mini tasks are satisfied, `status` and `fields summary` now stop suggesting another `evolve parallel` run and point to `evolve recommend field-mini-task` for the next harder mini task layer instead.
- Completed field-pool summaries now use that executable `evolve recommend field-mini-task` command as the actual `next_action`, including the active `--state` path, instead of mixing in plain prose.
- `field-mini-task` evolution now has a `field_pack_tasks` surface that targets `field-packs/*/field-pack.json`, so the harder-layer path can evolve task definitions as well as runtime repair templates.
- `field_pack_tasks` candidates now resolve that surface into the declared pack files plus `field-packs/index.json`, and provider-generated multi-file patches are accepted only when every diff path stays inside those resolved field-pack targets.
- LLM-generated `field_pack_tasks` patches are now prompted and tested as multi-file target-file patches, including `field-packs/index.json`, so the next-layer registry can evolve with the field definitions.
- The LLM evolution return schema now names `field-packs/...` as a valid target path for field-pack surfaces, so the planner is not steered back into tentacle-local files when the task surface lives outside the tentacle directory.
- Harder-layer recommendations for `field-mini-task` now select `field_pack_tasks`, so the command shown after a completed layer opens the task-definition surface instead of drifting back to runtime-code repair.
- Manual `evolve parallel` now also respects that completion state: if the selected candidate pool has satisfied the current mini-task layer, it opens zero worker slots, queues no Need, and returns the harder-layer next step instead of replaying old tasks.
- `field_parallel_pool` readiness now requires the named peer fields `math`, `search`, `code`, `swe`, `research`, `computer-use`, `ib`, and `robotics`; a random count of eight slots no longer passes product or preflight evidence.
- Real-machine preflight records now run `evolve parallel --workers 2 "preflight math and search field adaptation"` before `octopus status`, command coverage requires that exact math/search target, and `Field pool` evidence fails unless the filled result includes the required v0.2 named fields, `missing_required=none`, `latest_activity` with `worker=`, and `parallel_run` with requested slots, active slots, and a candidate pool containing `math` and `search`. Release evidence now proves recent field activity, worker identity, peer-field pool, worker capacity, and Goal-constrained candidate selection.
- Field trajectory reports now carry `latest_worker_slot_count` directly, so status and product report read the worker execution-slot count from the same trajectory contract instead of re-inferencing it.
- `fields summary` now prints the trajectory-level `latest_worker_slot_count` in English and Chinese, matching status/report/local app/desktop pet field-pool semantics.
- `status` now includes the field-pool worker policy in its compact line, so CLI, local App, and desktop pet all explain that workers are execution capacity.
- The local App status line now reads `field_pool.worker_slots` or `field_pool.policy`, plus the freshest slot's `latest_updated_at_secs`, so the browser observer explains worker capacity and the latest field activity instead of just showing slot counts.
- Peer-field worker Needs now carry structured queue context for `field_pack`, `field_mini_task`, and expected Feed before they enter Feed. Text markers remain for human observation and old state compatibility, but the live path no longer depends on parsing summary prose.
- Pending worker Needs now show compact `field=... task=...` labels in CLI lines and the local app, so the user can see which field worker is active without exposing long verifier expectations.
- Feed traces now preserve field metadata from Need context even when a tentacle forgets to echo it, so harness repair and field evolution can still group failures by field and mini task.
- Parallel worker records now carry `mini_task`, and CLI/JSON/native pet worker sources show the active task instead of only the field.
- Parallel worker `next_action` now follows run feedback: pending workers point to their queued Need, satisfied workers point to `fields summary`, and partial/failed workers point to `field-mini-task` harness repair for that field/task trace.
- `status` JSON and saved state now expose a read-only `field_pool` snapshot with peer slots, completed count, active slot, each field's mini-task progress, latest status, and next action. CLI status, desktop pet, and the local app can show the pool without parsing trace text.
- Saved `field_pool` next actions, including each slot's `next_action`, are now state-path aware, so external observers can display the exact follow-up command for the `.octopus/state.json` they are watching.
- Saving state refreshes `field_pool` from live traces and verifier results, and core state loading ignores serialized `field_pool`, keeping it an observer surface rather than a writable control source.
- `field-mini-task` is now the generic code-as-harness execution surface for peer-field worker tasks. It records task artifacts under `.octopus/field-mini-task/`, returns honest partial Feed when field behavior is not evolved yet, and gives harness repair a concrete trajectory to improve.
- `field-mini-task/repair-templates/` is now an editable runtime-template surface. The core loads task-specific `*.pyfrag` files from the active harness or repo template overlay; declared field mini task templates are migrated out of Rust core, `check field-mini-task` verifies declared-template coverage, compiles and executes each template in a temporary session, and live runtime Feed records the executed template path. The runner no longer keeps duplicate field-specific branches; concrete field behavior belongs to the editable harness templates. Field evolution candidates now target `repair-templates/<field>/<mini>.pyfrag` instead of inserting branches into `tools/run_field_mini_task.sh`; Rust tests assert that boundary instead of locking template internals into the core. Templates are standalone `if field/mini_task` files, so the runner no longer rewrites `elif` fragments at load time.
- `report` now exposes `field_mini_task_harness` as a product capability with the same `checked_count=...`, `executed_count=...`, `satisfied_count`, `partial_count`, `missing_count=0`, and `invalid_count=0` evidence used by preflight.
- `preflight` now includes a required `field_mini_task_harness` gate that runs `octopus check field-mini-task 2` and requires `checked_count=...`, `executed_count=...`, no missing templates, and no invalid templates. Honest `partial` templates are allowed only when they expose missing evidence instead of faking satisfied status.
- The field-mini-task harness evidence parser now lives in the release-gate module, so the CLI aggregation path only runs the check while release readiness owns declared-template coverage semantics.
...
- `repair` now resolves `harness-repair-agent` through the bundled seed overlay before installing and feeding it, keeping the self-repair path available in installed-user projects with missing or incomplete same-ID seed tentacles.
...
- Harness repair now writes `ACTION_TRACE_EFFECTIVENESS.md/json`, so reviewed outcomes teach whether the tool-side Need/Tool/Action/Feed trace actually helped later repair sessions.
...
- `repair continue` now returns a concrete `octopus repair score <trace>` command for the Feed it just ran, closing the heartbeat decision -> queued Need -> Feed -> scored repair outcome loop without manual trace replacement.
- `repair score latest` now resolves the newest Feed trace, matching `feedback latest` and `fields score latest` so the repair feedback loop can be scored without copying a trace id.
...
- Partial or failed repair-only scores now put `octopus repair continue .` first in the next steps when no evolution outcome is available, keeping negative feedback on the repair loop.
- `harness-repair-agent` now writes `score_options` into `REPAIR_PLAN.json` and REVIEW/COMMANDS artifacts, so the editable repair harness itself carries satisfied, partial, and failed human feedback paths.
- `harness-repair-agent` now writes `HARNESS_ADAPTATION.md/json`, compressing adapter state, field trajectory, repair memory, lesson effectiveness, and repair decision into a local environment-adaptation focus that heartbeat and `repair` reports expose without feeding tool burden to the clean brain.
- Repair heartbeat now queues the `HARNESS_ADAPTATION` next Need when a plan is not blocked, so local environment adaptation drives the next cognitive supply instead of leaving the adaptation plan as passive report data.
- Repair sessions now write `HARNESS_ADAPTATION_EFFECTIVENESS.md/json` from scored outcomes that used adaptation focus, then feed used/satisfied/failed counts and top reuse/avoid focus back into later harness adaptation plans.
- Repair sessions now write `HARNESS_ENVIRONMENT_PROFILE.md/json`, compressing local commands, runtimes, provider/desktop/GitHub availability, installed tentacles, and adaptation-effectiveness memory into a reusable harness profile that heartbeat and repair reports expose as Feed evidence.
- Repair sessions now keep `.octopus/harness-repair/environment-profiles.jsonl` and write `HARNESS_ENVIRONMENT_DRIFT.md/json`, so heartbeat can tell whether the local tool/provider/desktop substrate is baseline, stable, improved, changed, or degraded across repair runs.
...
- The required v0.2 fields are parallel Goal objectives. `math-mini-1`, `search-mini-1`, `ib-mini-1`, `code-mini-1`, `swe-mini-1`, `robotics-mini-1`, `research-mini-1`, and `computer-use-mini-1` now return `status=satisfied` and `verifier_status=satisfied`, each with a satisfied evolution outcome recorded for `field-mini-task/03-runtime-code`. The SWE trace exposed a real Python bytecode cache bug in the scratch test loop; the harness now clears `__pycache__` before regression rerun. The robotics trace now records a simulator-only path, collision check, contiguous step check, and no-hardware safety evidence. The research trace exposed a field marker bug where `research-mini-1` matched the `search-mini-` substring; field marker matching now uses a boundary check. The computer-use trace now records a local demo page observation, visible title, and check artifact. Queued field Needs carry field/mini-task markers so "table math" does not steal IB work into the math pack; `--workers 1` only sets the number of active execution slots.
- `octopus fields summary` now compresses field traces by parallel objective: trace counts, verifier counts, latest mini task, per-field unsatisfied mini task plus goal, pass evidence, repair signal, active slot field, and pool action. Each seed field pack now has three mini tasks. The current state reports all first-pass, second-layer, and third-layer tasks as satisfied across math, search, code, SWE, research, computer-use, IB, and robotics after real failure -> runtime patch -> authorized apply -> rerun -> verifier score -> evolution outcome cycles.
- `report` now includes a first-class field pool block and `field_parallel_pool` capability: peer field slots, completed count, active slot, latest field activity, and worker-slot policy are visible without reading raw field traces.
- `preflight` now has a required `field_parallel_pool` check, so release readiness proves the peer field slots exist before benchmark/provider gates are considered enough.
...
- Real-machine record templates and audits now include a required `Field pool` result, so release evidence must state whether the peer field slots were observed.
- Real-machine record templates and audits now include a required `Desktop pet source` result plus a `desktop_pet_source` preflight command, so the native observer compiler gate is visible in release evidence.
...
- Harness repair now writes `REPAIR_PATCH_VERIFY_EFFECTIVENESS.md/json`, so scored outcomes teach whether post-apply verification helped later repair sessions.
...
- The local app bridge now limits user-writable commands to `chat`, `goal set/refine`, and `first-run`; `brain --goal` stays a developer CLI surface, not a product app write path. Provider status/check and other diagnostics are observation-only.
- LLM-backed `chat` now queues clean cognitive Needs from Goal refinement while only running the memory Feed for the chat turn, so the first product path can move from Goal to Need without exposing tool controls or executing tools immediately.
- Queuing a clean Need now records a read-only `need` pet event, so the desktop observer can show head color and a Need bubble as soon as the brain produces a Need, before any tool action runs.
- Status reports now expose the latest pending Need rather than the last historical queue item, so app and CLI surfaces do not show already taken or dropped Needs as current work.
- Status `next_action` keeps pending Need as observation and returns to Goal/field-evolution product paths instead of exposing `needs run` or direct `need observe` as the first action.
- Need Queue reports keep execution as an internal review step: pending Needs remain visible, while default `next` points to Goal refinement, review session, or script export instead of direct `needs run/take/drop`.
- The HTML app now maps any pending Need to the pet `need` state even when the latest event is stale or unrelated, keeping the browser surface aligned with the desktop observer.
- CLI `pet auto` now uses the same pending-Need priority, so the static pixel pet, browser app, and native desktop observer agree when the brain has produced a Need.
...
- `preflight` now includes a required `bridge_goal_surface` gate that checks allowed Goal writes, denies internal writes including `brain --goal`, and verifies the structured denial policy.
- `bridge_goal_surface` evidence now denies Need Queue execution, Feed feedback, field scoring, OAuth grants, installs, checks, provider matrix runs, repair, update, and preflight record writes from the local app surface.
...
- `docs/about.html` now frames the product insight as a short public story with direct Local App/Tutorial actions, current runnable paths, and release evidence.
- `docs/recipes.html` now gives copyable Goal-first product recipes for local app startup, local repo startup, live model setup, and release evidence while keeping internal harness controls observational.
...
- Public quickstart/use docs no longer teach internal `brain --...` modes as user actions; they keep the first path to `first-run`, `chat`, `goal refine`, pet observation, and evidence surfaces.
- Public use and Chinese quickstart docs no longer teach direct `octopus need ...` plus `feedback latest` as the learning path; harness learning examples now stay on `first-run`, `chat`, `beat`, and report observation.
- README model setup is now a linked runtime-plumbing note instead of provider write commands, and Chinese quickstart now mirrors the English env-first model setup.
- README now drops Current Shape, docs index, and TODO detail, leaving the public entry as story plus Quick Install & Use.
- Public HTML docs and recipes now use env-first model setup too, so `provider save` remains developer/runtime plumbing rather than a public first-path write action.
- The five-minute use guide no longer includes the release provider matrix flow; matrix commands stay in release-gate docs, while the use guide stays Goal-first.
- Chinese homepage quick install now matches the English hosted installer path and keeps update/release plumbing out of the public first path.
- Quickstart, docs tutorial, and Chinese tutorial now include a short field evolution preview: one peer-field worker slot, one editable `field-mini-task` harness run, Feed plus verifier signal, and `fields summary` for review.
...
- Installed binaries now bundle `field-mini-task`, its runner, verifier, declared repair templates, field-packs, and minimal docs fixtures. If the current project has its own `tentacles/` without `field-mini-task`, `evolve parallel` and `check field-mini-task` fall back to the bundled seed so field Feed and field checks can run without a source checkout.
- `evolve parallel --workers n` now validates worker slots as 1 to 8 execution slots, so bad input cannot silently fall back to the default parallel run.
- `needs run --workers 1..8` now shares the same worker-slot validator and next-step helper, so internal queued-Need execution cannot bypass or suggest work outside the peer-field execution boundary.
- Legacy `needs run all` / `--all` is hidden from usage and capped at 8 queued Needs for compatibility, keeping the product path on explicit worker slots.
- `needs run <selector>` now rejects invalid selectors and extra arguments before Feed, so a typo cannot silently execute the first queued Need.
- The core parallel-field detector now recognizes public `peer field`, `peer-field`, `field adaptation`, `field-adaptation`, and `field evolution` wording from README/docs. The public API, `evolve parallel --workers`, `pet desktop --workers`, and the desktop pet launcher all reject worker limits outside `1..8` before any Need, run record, platform check, or desktop observer is started.
...
- README, quickstart, and docs tutorial now put `start --check` before the first Goal loop, then show `pet desktop` and `start --open` after state exists, so the public first path does not block in a long-running app server.
...
- `start --check` report `next` now starts with `first-run`, then `start --open`, then `preflight`, so successful startup evidence returns to the Goal path before diagnostics.
...
- `update` reports the current GitHub reinstall command by default, and `update --run` performs the cargo reinstall explicitly from CLI/internal paths; the app bridge denies update/install writes.
...
- `repair continue [query]` runs repair, takes the queued clean Need, routes it back through the harness, and returns the continued Feed plus trace-scoring commands; `--score <status> [summary]` records the reviewed repair outcome immediately.
...
- `preflight record [path]` writes a real-machine evidence template with GitHub install, download artifacts, core loop, field pool, field-mini-task harness coverage, `start --check`, start/app, live provider, and PR dry-run commands; `preflight record check [path]` audits completed fields, including `missing_count=0` and `invalid_count=0` for the field-mini-task harness, before appending it.
...
- `preflight record append [path] [log]` appends a passed real-machine record to `docs/real-machine-test.md` idempotently from the release CLI/internal path.
- Real-machine record audit accepts the current `Start/app` result field while keeping legacy `Bridge/app` records valid.
- `0.0.24` is now reserved for a full core audit gate that removes redundant development residue, misaligned local substitutes, and code that belongs in editable harness units before `0.1.0`.
...
- Repair sessions now write `REPAIR_EFFECTIVENESS_ROLLUP_EFFECTIVENESS.md/json`, so rollup guidance has its own reviewable success/failure Feed before heartbeat acts.
- Repair sessions now write `REPAIR_EFFECTIVENESS_ROLLUP_ADAPTATION.md/json`; heartbeat can let it queue the next Need only when rollup guidance itself is unreliable.
- Repair outcomes now preserve repair-effectiveness-rollup usage in `OUTCOME.md`, `outcomes.jsonl`, and later `OUTCOME_MEMORY.md`; the next rollup treats prior rollup guidance as one scored repair source.
...
- Rebuilt `pet.html` as a pixel Octopus whose body pixels change color; CLI/chat badge returns colored squares.
...
- Added provider-backed harness-beat evolution recommendations so `beat` can write the next apply plan from failed or partial check history only when `OCTOPUS_LLM_EVOLVE=1`.
...
- Added Feed-trace-driven harness beat evolution so failed or partial Feed can produce the next reviewed apply plan through the configured LLM planner without waiting for a manifest check failure.
...
- Rolled the cleanup/version cadence to `0.0.24` after the full core audit, hidden planner removal, explicit seed tool contracts, CLI split for bundled harness/download/pet/profile-registry support, shared shell display quoting, and boundary report alignment.
...
- Rolled the `0.1.1` repair cleanup pass by aligning version-plan state and consolidating repair effectiveness report labels after the patch apply/verify learning cycle.
...
- Rolled the cleanup/version cadence to `0.0.17` after product path cleanup, bridge clarity, readiness diagnostics, core/harness boundary evidence, product use guide, docs/static asset cleanup, and version consistency.
...
- Rolled the cleanup/version cadence to `0.0.12` after local app startup, pixel pet SVG export, clean-brain clarification, reflection, memory, council, synthesis, provider slots, native app controls, embedded HTML app assets, README/version consistency, and cleanup scan.
...
- Embedded native HTML pages into the Rust binary so installed binaries can serve the app even when source docs are unavailable.
...
- Added `octopus update` as a dry-run-first update path, with `--run` for explicit GitHub reinstall; the local app no longer exposes this install/update path.
...
- Added field/task labels to parallel-evolution action pet events, so the desktop pet can show which peer field slots start running before their Feed returns.
- Fixed provider-matrix and benchmark record refresh behavior so rerunning the gate on a new head updates the recorded commit/version without erasing existing provider or benchmark evidence.
- Verified a real local OpenAI-compatible provider path with `llama-server` and the local Qwen GGUF: direct provider check passes with thinking disabled. Full local matrix still needs a smaller release-gate case because the 35B CPU path is too slow for clean-brain, tentacle-planning, and evolution checks in the synchronous gate.

## Remaining Gaps

- Self-iteration now has an OAuth-scoped PR adapter and a recorded dry-run PR path; live `gh` publishing still needs feedback.
- LLM evolution can generate candidates and provider-assisted patch drafts from manifest surfaces, scored outcomes, recent Feed traces, and check history. Local candidate generation is removed; `evolve score` and harness beat can only write the next recommendation/apply artifacts from LLM-generated candidates while app and CLI patch writes stay review/grant-bound.
...
- Provider matrix is now current-head accurate and Codex OAuth passes all four layers. Local OpenAI-compatible provider check passes against a real Qwen GGUF server, but the full local target still needs a lighter matrix prompt/case and the API-key cloud plus gateway targets still need real enabled providers before `0.2.0`.
- GitHub `cargo install` now works with current Cargo syntax; non-Rust packaging still needs finish.
- The HTML app first path is now narrowed to Goal, Need, Feed, pet, and Output; deeper provider, repair, install/check, preflight, and harness evolution surfaces remain CLI/docs/internal evidence.
