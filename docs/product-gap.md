...
- `status` and `report` expose a compact harness-learning summary from repair/evolution outcomes, including target tentacle, candidate, score, summary, and next recommendation command.
...
- Added status/report harness-learning summaries so repair-driven evolution evidence is visible without reading raw state JSON.
