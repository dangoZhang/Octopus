...
- `benchmark record [path]` writes the smallest required benchmark evidence template for SWE-Bench verify mini 1-3, Claw-SWE-Bench simplest, and Wild-Claw-Bench simplest; `benchmark check [path]` audits filled pass evidence, and `preflight` treats missing benchmark evidence as a required release blocker.
