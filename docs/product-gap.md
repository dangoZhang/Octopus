...
- `evolve parallel` now treats fields named in the objective as the current candidate pool, so a targeted math/search run opens worker slots only inside that peer subset while the default run keeps all eight fields available.
...
- Parallel evolution runs now also persist `field_pool_size` and `field_pool_policy`, so observer tools can distinguish the eight peer field pool from the number of active worker execution slots.
- Status `field_pool` now exposes machine-readable `field_slot_count` and `latest_worker_slot_count`; the local app and desktop pet read those fields before older `field_count` fallbacks.
...
- `report` and `preflight` now share one field-pool summary line, so product evidence names both the eight peer slots and the latest worker execution-slot count.
...
- `field_pack_tasks` candidates now resolve that surface into the eight concrete pack files plus `field-packs/index.json`, and provider-generated multi-file patches are accepted only when every diff path stays inside those resolved field-pack targets.
...
- `status` JSON and saved state now expose a read-only `field_pool` snapshot with eight peer slots, completed count, active slot, each field's mini-task progress, latest status, and next action. CLI status, desktop pet, and the local app can show the pool without parsing trace text.
...
- `report` now exposes `field_mini_task_harness` as a product capability with the same `checked_count=24`, `executed_count=24`, `missing_count=0`, and `invalid_count=0` evidence used by preflight.
- `preflight` now includes a required `field_mini_task_harness` gate that runs `octopus check field-mini-task 2` and requires `checked_count=24`, `executed_count=24`, no missing templates, and no invalid templates.
- The field-mini-task harness evidence parser now lives in the release-gate module, so the CLI aggregation path only runs the check while release readiness owns the 24/24 coverage semantics.
...
- `report` now includes a first-class field pool block and `field_parallel_pool` capability: eight peer field slots, completed count, active slot, latest field activity, and worker-slot policy are visible without reading raw field traces.
...
- `needs run --workers 1..8` now shares the same worker-slot validator and next-step helper, so internal queued-Need execution cannot bypass or suggest work outside the eight peer-field execution boundary.
