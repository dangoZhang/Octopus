...
- Harness-repair diagnosis can now queue its next Need, immediately continue that Need as Feed, remember reviewed session outcomes, feed merged outcome memory, target code context, and field trajectory context into repair sessions, write reviewable action plans, continue those plans through heartbeat repair, expose the trajectory and reviewed outcome as review targets, accept repair scoring from CLI or the native app, mirror session-backed scores into the outcome journal, and feed failed or partial repair outcomes into harness beat evolution; closing the loop still needs real provider-backed repair runs scored from real projects.
